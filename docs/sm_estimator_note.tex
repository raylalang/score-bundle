...
\begin{center}
\fbox{\parbox{0.92\linewidth}{\small\textbf{Outcome (2026-09-04, added
2026-09-16).} The evaluation of \S7 was run as committed
(5{,}157 development notes; \texttt{results/sm\_estimator\_dev.md}). By
the pass/fail rule the GP estimator was \textbf{not adopted}: it does not
beat the sine fit on parameter accuracy (intonation ties; extent and rate
lose decisively --- the evidence's decomposition is less reproducible
across two measurements of the same note than the rigid fit). The
curve-level measure (4) favoured the GP --- the honest two-sided reading
(2026-09-18 stratified addendum): the GP is the better model of the
curve, and the less stable producer of the sine-defined scalars the
pipeline consumes; the swap is closed, the interface question is opened.
The follow-up at the waveform level
(\texttt{results/phase3\_smprior\_dev.md}) measured no gain at matched
rank. This note is kept as the design record; the sine estimator remains
...
lags of order $1/\sqrt{v_1}$. Consequences: (i) $w_1$ converts to an
amplitude via $\gamma = \sqrt{2 w_1}$ --- but note $w_1$ is the
\emph{ensemble} scale: on a near-coherent component it is informed by only
two effective degrees of freedom (the realized sine and cosine
amplitudes), so $\sqrt{2\hat w_1}$ is $\chi^2_2$-spread around the
\emph{realized} amplitude of this note, which is the quantity of interest;
the extent read-out therefore comes from the posterior vibrato component
(\S4), not from $\hat w_1$; (ii) the classical parametric fit (constant
plus phase-locked sinusoid) is the degenerate sub-model $v_1 \to 0$,
$w_2 = 0$, with the phase treated as known rather than marginalized.
...
Two identifiability bounds are imposed on the search, each closing a real
flat direction of $L$. The drift bandwidth is bounded below,
$2\pi^2 v_2 T^2 \ge \tfrac12$ over the note span $T$: a drift component
with $v_2 \to 0$ is a pure constant, exactly redundant with the
flat-prior $c$ (the evidence is flat in that direction and $w_2$ can run
away). A drift must vary within the note; the constant belongs to $c$.
Generous caps additionally bound the powers ($\le 10^3\,
\widehat{\operatorname{Var}}[y]$) and bandwidths.
...
extent & $\hat\gamma = \sqrt{2}\,\operatorname{std}_j \hat m_1(t_j)$ &
$\hat\gamma^2 \cdot \tfrac14 [H^{-1}]_{\log w_1 \log w_1}$
\quad (scale uncertainty via $w_1$) \\
& \multicolumn{2}{l}{\quad with $\hat m_1 = K_1 K^{-1} (y - \hat c \one)$
the posterior vibrato component} \\
\end{tabular}
\end{center}
