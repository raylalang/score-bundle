...
\newcommand{\rmseStack}{0.388}         % features + network + graph (candidate)
...
The known score is $\mathcal{S}=\{s_i\}_{i=1}^N$ with $s_i=(p_i,b_i,d_i)$ (pitch
degree, beat onset, beat duration). Each note carries a Phase-1 performance
vector
\[
...
With $\tilde y = y + e,\ e\sim\mathcal N(0,\Sigma_e)$, the posterior is Gaussian,
\[
  p(y\mid\tilde y)=\mathcal N(m,\ \Sy),\quad
  \Sy=(Q_G+\Sigma_e^{-1})^{-1},\quad
  m=\Sy(Q_G\,\mu+\Sigma_e^{-1}\tilde y).
\]
Held-out notes carry no likelihood term and are predicted from their neighbours.
\TODO{predictive variance = latent + observation noise (the corrected form).}
...
Prior hyperparameters are learned by (restricted) marginal likelihood per piece.
Two safeguards: a floor on the observation-noise level, and a self-validating
guard that falls back to a conservative fit when the fitted field fails on a
held-back sliver of observed notes. \TODO{state both precisely.}

% =====================================================================
\chapter{Data and Experimental Protocol}\label{ch:data}

\section{Pretraining corpus (MAESTRO)}
\TODO{size, split, tokenisation; only MAESTRO MIDI is used for pretraining.}
...
twin was in pretraining is removed ($\approx$1000 $\to$ $\approx$650), so the
model is tested only on unseen performances. A \emph{strict} protocol
additionally blanks every hidden note's loudness from the input.
\TODO{exact counts; cost of strictness (essentially nil).}
...
\caption{Held-out ASAP, strict protocol. Lower RMSE/NLL is better; coverage
closer to 90\% is better. \TODO{verify against docs/phase1\_calibration\_results.md;
add NLL and CIs.}}
\begin{tabular}{lccc}
\toprule
System & RMSE & NLL & coverage \\
\midrule
Predict zero                 & \rmseZero     & \TODO{} & \TODO{} \\
Graph alone                  & \rmseGraph    & \TODO{} & \TODO{} \\
Network + graph (headline)   & \textbf{\rmseNetGraph} & \TODO{} & \covHead \\
Features + network + graph   & \rmseStack    & \TODO{} & \TODO{} \\
\bottomrule
\end{tabular}
\end{table}
...
corrected advantage is smaller but honest and statistically significant.
\TODO{before/after loudness numbers (band-aid $\to$ leak-free $\to$ flawed claim).}

\section{Hyperparameter fragility and the guard}
On a minority of evaluation cells the noise fit collapsed toward zero
(overconfident); a noise-level floor plus a self-validating guard fix it, shown
harmless on healthy runs (bit-for-bit identical). \TODO{numbers; the 1-in-120 cell.}
...
did \emph{not} beat the original at matched compute (3$\times$ budget does not
close it). The honest lesson: the original read-out already extracted what the
objective change was meant to unlock. \TODO{table; the reintroduced-leak catch.}

\section{Scaling: a bigger LM does not help}
A 1.3$\times$ larger, lower-perplexity model gives an essentially identical
downstream result once the leak is removed --- the earlier ``scaling'' gain was
the loudness leak. Practical rule: judge the LM downstream, not by perplexity.
\TODO{numbers.}
...
better (AUROC \TODO{}) and its edge \emph{grows} as errors get subtler.
...
per-note shrinkage on both error and calibration. \emph{Honest negative}:
estimating the noise level from a single piece is unreliable. \TODO{numbers.}

\section{Completion / rendering from an excerpt --- partial}
Predict the expression of the rest of a piece from its opening. The LM guess
carries the far-future; the graph helps only where hidden notes have nearby heard
neighbours. \TODO{numbers.}
...
for articulation and for catching the model's own misfires. \TODO{numbers.}

\section{Musical-era classification --- honest negative}
Style statistics alone do not beat the majority-class baseline; the graph, a
note-level tool, does not help whole-piece summaries. \TODO{numbers.}

\section{Performer identification (Vienna 4x22) --- mixed}
The extracted expression \emph{does} carry a pianist's identity
(3--4$\times$ chance), validating our extraction; graph smoothing does not help
this whole-performance summary. \TODO{numbers.}
