...
\title{\textbf{Expressive Performance as a Gaussian Process\\ on the Score Graph}\\[2mm]
  \large calibrated per-note timing, articulation, and dynamics}
...
\begin{frame}{Related work}
  ...
  \begin{itemize}
    ...
    \item multi-output coupling via the intrinsic coregionalization model
          (Bonilla et al.\ 2008)
  \end{itemize}
  \end{columns}
  \vspace{3mm}
...
\begin{frame}{Experiment setup}
  \begin{itemize}
    \item \textbf{Data}: ASAP piano (aligned score + performance MIDI),
          filtered so no test performance appeared in pretraining
          ($1036 \to 653$).
    \item \textbf{Split}: 40 pieces to fit cross-piece components; 30 for all
          development; \textbf{20 test pieces evaluated exactly once}, with the
          claims fixed beforehand; 30 more later as a replication set.
    ...
    \item \textbf{Metrics}: RMSE; NLL, coverage, and PIT for the error bars.
          A method must improve both to count as better. All comparisons are
          paired per piece, with bootstrap CIs over pieces.
    \item \textbf{Baselines}: independent notes, temporal smoothing, feature
          and embedding regressors, and a \textbf{two-stage baseline}: fit a
          mean predictor across pieces, then smooth its residuals with the
          graph, one channel at a time.
  \end{itemize}
\end{frame}
