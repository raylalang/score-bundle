...
\begin{frame}{Recent work: three measured answers}
  \begin{enumerate}
    \item \textbf{The spectral mixture kernel direction}, measured at
          both points where it could enter the pipeline: the better
          model of the pitch curve. The swap at the current scalar
          interface is off by a pre committed rule, and the measured
          bottleneck is the interface, not the model. The direction
          stays live.
...
\begin{frame}{The kernel direction: measured at both entry points}
  ...
  \end{enumerate}
  \vfill
  The reading: the model outgrew the scalar interface, not the other way
  round. The direction stays live. The principled next designs:
  marginalize the rate instead of point-picking it, or move the channels
  themselves to curve level, where the two GP levels meet directly.
\end{frame}
...
\begin{frame}{Open decisions}
  \begin{enumerate}
    ...
    \item \textbf{The kernel direction, the main discussion.} The
          spectral mixture is the measured better model of the curve,
          and the scalar interface is the measured bottleneck. Ways
          forward: (a) marginalize the rate and retest at the same
          interface, (b) move the channels to curve level so the
          within-note and across-note GPs meet directly, (c) register
          the timing claim first and return. Sequencing, not a keep or
          ditch decision: (c) first does not close (a) or (b).
  \end{enumerate}
\end{frame}
