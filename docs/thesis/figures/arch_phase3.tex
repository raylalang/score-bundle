% arch_phase3.tex — Phase 3 (waveform likelihood; not yet implemented), "architecture-diagram" style (isometric volumes, two branches).
% Standalone TikZ; compile with:  pdflatex arch_phase3.tex
...
\node[dim, below=1.5mm of hhs] {$H$ (embeddings)\\[1pt] $N\!\times\!512$};
...
\node[op=ggreen, text width=16mm, fill=ggreen!8, dashed] (noise) at (14.6,-3.6) {no per-note noise\\ \tiny (no $\tilde y$ in Phase 3)};

\cuboid{16.25}{-0.85}{1.15}{1.45}{0.34}{ggreen}{kk}
\node[dim, below=1.5mm of kks] {prior $p(z)$: structured part\\[1pt] $B\otimes K_G+\textstyle\sum_f\operatorname{diag}(c_f)\otimes X_fX_f^{\!\top}$};

% ================= HEAD =================
\node[op=gpink, text width=24mm, text=pinkink, dashed] (head) at (19.55,-0.1)
  {\textbf{non-conjugate}\\[1pt] Laplace / VI over $z$\\[1pt] \tiny $a$ recovered in closed form};
...
\begin{scope}[shift={(15.9,-4.9)}]
  \draw[gline, line width=0.6pt, domain=0:1.5, samples=110, smooth, variable=\u]
        plot ({\u},{0.30*sin(720*\u)*exp(-0.7*\u)});
  \coordinate (obe) at (1.55,0);
\end{scope}
\node[dim] at (16.65,-5.55) {waveform $x\in\mathbb{R}^{M}$\\[1pt] \emph{no} performance MIDI};

\node[op=gverm, text width=30mm, line width=1.1pt, text=gverm, dashed] (lik) at (19.75,-4.4)
  {\textbf{audio likelihood}\\[1pt] \tiny $x=\Phi(z)a+\varepsilon$\\[-1pt] \tiny amplitudes $a$ marginalised exactly:\\[-1pt] \tiny $p(x\mid z)=\mathcal N(\Phi\mu_a,\ \Phi\Sigma_a\Phi^{\!\top}\!+\!K_n)$};
\node[sec=gverm, above=1.2mm of lik] {LIKELIHOOD\ \ ---\ \ CHANGES\ \ WITH\ \ THE\ \ PHASE};

% ================= OUTPUT : estimates with error bars =================
\begin{scope}[shift={(22.3,-0.1)}]
  \foreach \x/\v/\e in {0/0.50/0.34, 0.45/-0.22/0.26} {
    \draw[gline, line width=0.7pt] (\x,{\v-\e}) -- (\x,{\v+\e});
    \draw[gline, line width=0.7pt] ({\x-0.06},{\v-\e}) -- ({\x+0.06},{\v-\e});
    \draw[gline, line width=0.7pt] ({\x-0.06},{\v+\e}) -- ({\x+0.06},{\v+\e});
    \fill[gink] (\x,\v) circle (0.05);
  }
  \draw[gverm, line width=0.6pt, domain=0:1.3, samples=100, smooth, variable=\u]
        plot ({0.85+\u},{0.26*sin(700*\u)*exp(-0.55*\u)});
\end{scope}
\node[dim, text=gink] at (23.1,-1.35)
  {posterior over \emph{every} note's $z$\\[1pt] $+$ amplitudes; resynthesis};
