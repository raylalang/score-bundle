...
\node[lik] (l2) at (7.6, 0)
  {\textbf{Phase 2: $f_0$-derived targets}\\[1pt]
   noisy estimates, six channels,\\ per-(note, channel) masks};
\node[lik, dashed] (l3) at (7.6, -2.3)
  {\textbf{Phase 3: waveform}\\[1pt]
   $x = \Phi(z)\,a + \epsilon$, no per-note data};

\node[tag, fill=okgreen, anchor=east] at ([xshift=-2mm]l1.north east) {Confirmed};
\node[tag, fill=okgreen, anchor=east] at ([xshift=-2mm]l2.north east) {Confirmed};
\node[tag, fill=okamber, anchor=east] at ([xshift=-2mm]l3.north east) {Scoped};
