% arch_phase2.tex — Phase 2 (the real-audio six-channel bundle; running at development level, claims registered), "architecture-diagram" style (isometric volumes, two branches).
...
\node[op=gamber, text width=17mm] (mm1) at (3.30,-3.74) {music model\\ \tiny decoder-only, $\times L$\\ \tiny (piano-trained; unused here)};
...
\node[op=ggreen, text width=16mm, fill=ggreen!8] (noise) at (14.6,-3.6) {$\Sigma_e=\operatorname{diag}(\sigma^2_{e,ij})$\\ \tiny heteroscedastic noise};
...
\node[dim, above=1.6mm of head, text width=30mm]
  {$\theta=(s,B,\{c_f\},\text{noise scale})$ learned by one\\ exact per-piece marginal likelihood};

% ================= LIKELIHOOD (the changing part) =================
\node[op=ggray, text width=17mm] (aud) at (13.75,-5.95) {monophonic audio\\ \tiny $f_0$ tracking $+$ annotated\\ \tiny onsets $\to$ warp};
\cuboid{16.15}{-5.35}{0.7}{0.95}{0.24}{ggray}{ob}
\node[dim, below=1.4mm of obs] {per-note cells, each with variance\\[1pt] $c,\ \log\gamma,\ \log f^{\mathrm{vib}},\ \ell,\ \tau,\ \delta^{\mathrm{vib}}$};

\node[op=gverm, text width=24mm, line width=1.1pt, text=gverm] (lik) at (19.55,-4.4)
...
\node[dim, text=gink] at (23.05,-1.35)
  {the six-channel bundle\\[1pt] $[c, \log\gamma, \log f^{\mathrm{vib}}, \ell, \tau, \delta^{\mathrm{vib}}]$\\[1pt] every cell with uncertainty};
