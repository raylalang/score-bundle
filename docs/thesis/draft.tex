...
unchanged, to intonation, vibrato, and loudness on continuously pitched
instruments: on real recordings (URMP, development set) the full pipeline ---
tracker, per-note estimator, per-(note, channel) missingness, heteroscedastic
likelihood --- runs end to end, and the graph prior improves held-out recovery
on every channel with calibrated intervals; its confirmation set is
preregistration-ready and untouched. Phase~3 (audio via a differentiable
...
$f_0$-derived channels ($c_i$ cents, vibrato, $\ell_i$) & \textbf{Running on real data} (URMP, development): graph wins every channel; confirmation not yet registered \\
...
same-voice bonus $w_v \ge 0$. The (combinatorial) graph Laplacian is
\begin{equation}
...
$s' = 1/s$), also known as the regularized-Laplacian kernel.
...
Observation noise is not a fourth component here, by independence rather than
by convention: a held-out note's noise is never observed and is independent of
every observation, so its conditional mean is identically zero. Noise enters
\eqref{eq:decomp} only through the shared weights --- $K_{oo}$ contains
...
comparable to the components' posterior variances.
The posterior covariance decomposes likewise, with
...
the MIDI velocity $v_i$: with the note's audio span split into $C = 4$ equal
chunks of samples $x^{(k)}$,
\begin{equation}
  \ell_i \;=\; \frac{1}{C} \sum_{k=1}^{C} \log \mathrm{rms}\bigl(x^{(k)}\bigr)
  \;-\; \overline{\ell}_{\mathrm{track}},
  \qquad
  \widehat{\operatorname{Var}}[\ell_i] \;=\; \frac{s^2_{\mathrm{chunks}}}{C},
  \label{eq:loudness}
\end{equation}
centred per recording because absolute level is arbitrary per track, with the
chunk variance supplying its estimator noise; $c_i$
...
supplies. The observation rule, with every threshold fixed by the
development measurements of \S\ref{sec:phase2-real}: note $i$'s sample set is
\begin{equation}
  V_i \;=\; \bigl\{\, \text{frames } j:\; t_j \in [t_i,\, t_i + \delta_i),\;
  \text{voiced}_j,\; p_j \ge q_{0.2}(\text{track}) \,\bigr\},
  \label{eq:framerule}
\end{equation}
(the tracker's per-frame confidence $p_j$ thresholded at the track's lowest
quintile — the measured gross-error filter); the intonation cell is observed
iff $|V_i| \ge 4$ \emph{and} $|\hat c_i| \le 150$ cents (the ground-truth
-validated octave-failure rule); the vibrato cells additionally require the
identifiability rule below. Remaining open choice: a robust loss rather than
least squares inside the fit.
...
data re-confirmed this the hard way (\S\ref{sec:phase2-real}): with estimator
...
score-onset/performed-onset pairs $(b_j, t_j)$,
\begin{equation}
  \hat T_{-i}(b) \;=\; \hat a_i\, b + \hat u_i, \quad
  (\hat a_i, \hat u_i) = \arg\min \textstyle\sum_{0 < |j - i| \le 8}
  \bigl(t_j - a\, b_j - u\bigr)^2, \qquad
  \tau_i = t_i - \hat T_{-i}(b_i),
  \label{eq:localwarp}
\end{equation}
so each note's residual is judged against a tempo line fit from its
neighbours alone. The result: median $\operatorname{std}(\tau)$ of $79$\,ms
with lag-1 neighbour correlation $+0.59$ --- Phase-1-scale timing with
exactly the structure the graph prior models, so $\tau$ is viable as a
Phase-2 channel here (development evidence; the channel is not yet part of
...
strings, woodwind, and brass). Three preparatory measurements discipline its
use. (i) The tracker was calibrated against the provided ground-truth pitch
...
errors (gross octave slips fall from $9.6\%$ to $0.4\%$ across confidence
...
data-blind (13 of 44 pieces in the untouched confirmation pool) --- and this
...
independent quasi-truth cross-check, used below). The claim available in
...
f^{\mathrm{vib}}]$ plus per-note loudness $\ell$ (log RMS, centred per
recording), estimator variances as given, per-(note, channel) missingness
from the identifiability rule (55--93\% of notes identifiable per track ---
the missingness case is the norm, not an edge case), and the heteroscedastic
cell-mask GP --- on 77 unique development tracks (shared recordings
deduplicated), hiding 30\% of notes. One target-quality rule was added with
measured justification rather than by fiat: $2.2\%$ of intonation targets
...
they are marked missing --- the same epistemic status as unidentifiable
vibrato. The result: \textbf{the graph prior improves held-out recovery on
every channel of the bundle, with calibrated intervals} --- paired against
the no-graph ablation, $\Delta$RMSE $-0.92$ cents (intonation), $-0.15$
(log extent), $-0.17$ (log rate), $-0.010$ (loudness), all significant,
coverage $0.88$--$0.91$ at nominal $90\%$ --- and the ordering is unchanged
when scored against the quasi-truth targets derived from the provided pitch
curves. The win holds \emph{within each instrument family} (strings,
woodwind, brass; the single non-result is loudness on woodwinds), so no
family carries the average. Estimator-missing vibrato cells on visible notes
...
Phase-1 sense: it selected the design defaults above, and any confirmed
Phase-2 claim awaits the frozen confirmation pool.

\figph{phase2_real_example.png}{1.0}{Phase 2 on one real development track
(violin), 30\% of notes hidden. Top: intonation --- the estimator's
per-note values with their own uncertainties (dots), the cell-mask GP's
posterior with its 90\% predictive band, and the hidden notes' estimator
values (open circles). Bottom: vibrato extent --- squares mark notes whose
extent the estimator could not identify from the audio; the GP supplies a
posterior there anyway, informed by neighbours through the graph and by the
coupled channels.\label{fig:phase2-real}}
...
designed for --- now shown on real recordings at development level, with the
confirmation-grade version deliberately still unspent.
...
posterior \emph{mean} at held-out notes,
$s^{(\cdot)} = \operatorname{Cov}\bigl(m^{(\cdot)},\, m\bigr) / \operatorname{Var}(m)$
per channel with the components of \eqref{eq:decomp} --- an allocation that
sums to one exactly, cross-terms landing where they belong --- put
...
posterior cross-correlations between components --- the per-note diagonal of
the cross-covariance of \S\ref{sec:posterior}, normalised by the two
component posterior standard deviations, averaged over held-out notes ---
...
on loudness the embedding component tracks the note-to-note alternation while
...
component nearly flat outside loudness.\label{fig:posterior-components}}
...
minority --- and at the extremes the division of labour inverts wholesale.
One piece's loudness rests on the graph (share $0.76$; its level and a
...
and $15\%$ on loudness --- the per-note face of the $-0.19/-0.18$
cross-correlations above --- and least often between graph and embeddings on
timing ($6\%$): such notes are exactly where the per-piece evidence had to
arbitrate between overlapping explanations, and where the decomposition is
most informative to inspect.

\figph{posterior_components_contrast_dev.png}{1.0}{The same channel
(loudness), two development pieces, opposite division of labour --- dominance
...
loudness --- timing contributes little either way. The cross-channel coupling
itself is also a loudness story: refitting the full model with $B$ constrained
diagonal costs velocity specifically ($+0.0075$ RMSE / $+0.093$ NLL, paired,
development, significant) and changes timing and articulation by nothing
material --- the coregionalization earns its keep where an observed loud
...
25-feature baseline is loudness; timing and articulation tie (development) ---
...
pieces at each level but never significantly (the means are disturbed by the
documented Gaussian-tail piece, \S\ref{sec:guard-gp}). At the 40\% operating
point it ties (the adoption record), and at 50\% hidden the advantage is
gone ($-0.0037$, ns) --- and that rate also produced the one fit collapse of
the sweep (one cell; the graph-parameter optimiser wandered to degenerate
length scales, and the safeguard's calibration screen could not see it from
the observed notes, the known blind spot of \S\ref{sec:guard-gp}). The
...
music-model components act nearly orthogonally per note (cross-correlation
...
three now have measured answers:
\begin{enumerate}[leftmargin=*,noitemsep]
  \item \textbf{Per-channel missingness --- resolved in code and exercised
        ...
        unit-pinned exact, and on real tracks 7--45\% of notes are in the
        missingness regime.
  \item \textbf{Where alignment error goes --- measured.} On URMP the
        ...
        neighbour correlation), so $\tau$ is viable as a channel there;
  ...
  \item \textbf{The loudness channel --- in the bundle, commensurability
        still a claim.} $\ell_i$ (log RMS) is now measured and the graph
        improves it, but whether it may be discussed as one channel with the
  ...
  \item \textbf{What a result would mean.} Unchanged, and load-bearing: the
        ...
        Phase-1 claim.
\end{enumerate}
What still blocks a confirmable Phase-2 result: adopting $\tau$ into the
evaluated bundle, the vibrato-delay ($\delta^{\mathrm{vib}}$) in/out
decision, and freezing the claim set for the preregistered confirmation
pool (13 pieces, untouched).
...
\figph{arch_phase2.pdf}{1.0}{Phase 2 (dashed blocks were prospective when
drawn; the pipeline now runs on real data at development level,
\S\ref{sec:phase2-real}). Audio enters only to \emph{derive} per-note
targets ($c_i$ cents, vibrato, loudness). One structural block changed
...
been carried, unchanged in form, to intonation, vibrato, and loudness on real
recordings --- at development level, with a frozen confirmation pool
...
$m^{(G)}$, $m^{(f)}$, $s^{(\cdot)}$ & posterior-mean components \eqref{eq:decomp}; covariance share of a component (\S\ref{sec:attribution}) \\
