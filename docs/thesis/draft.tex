...
DTW), and the residual timing under a local leave-one-out warp has median
scale $79$\,ms with lag-1 neighbour correlation $+0.59$ --- Phase-1-scale
timing with exactly the structure the graph prior models, so $\tau$ is
viable as a Phase-2 channel here (development evidence; the channel is not
yet part of the evaluated bundle).
