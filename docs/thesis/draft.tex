...
same-voice bonus $w_v \ge 0$ (untuned defaults throughout: $\ell_b = 2$,
$\ell_p = 4$, and $w_v = 0$ --- ASAP carries no voice labels, so the voice
bonus is inert in every reported run and stands as the documented hook for
voiced corpora). The (combinatorial) graph Laplacian is
\begin{equation}
...
proximity, deliberately unlike semitone proximity. Substituting it for the pitch term (the voice bonus is dropped and an octave-band locality term added alongside)
...
under the joint reparameterization $s = \alpha/t$ (at fixed $s$ the limit
degenerates to zero).
...
log-Cholesky factor. Because $g(0) = 1$, the split of scale between $B$ and
$K_G$ is identifiable and $B$ carries all of it --- the per-note prior variance
...
\medskip\noindent
Each feature matrix has its columns z-scored within the piece, with a bias
column appended, before entering the kernel --- the single per-channel scale
$c_{f,c}$ is isotropic over columns, so the standardization is load-bearing,
not cosmetic.
...
only in the joint model's safeguard --- the two are related, not identical.
...
with one spectral shape $s$ common to the channels, the joint marginal
likelihood \eqref{eq:marglik} equals the sum of the per-channel ones under
$(\lambda_c, \eta_c) = (1/B_{cc},\, s/B_{cc})$, and the equality is pinned
by a unit test. The per-channel pipeline, fit independently, may select
three different shapes; the nesting statement is about the model
\emph{family} --- every per-channel configuration with a common shape is a
point of the joint parameter space --- not about coinciding optima. The constrained systems are therefore not
...
claim set: recovery on intonation; calibration on \emph{both} vibrato
channels; coverage within $[0.85, 0.95]$ on all six channels; and timing
calibration as a secondary claim that does not gate the others --- the
delay channel deliberately carries no claim, and the rule is one-shot with
...
survives every classical alternative profile}: Mat\'ern at every $\alpha$ (at $\alpha=1$ the Mat\'ern profile \emph{is} the additive baseline under $s \mapsto 1/s$, so only $\alpha \ge 2$ are distinct alternatives), diffusion,
...
\begin{enumerate}[leftmargin=*,noitemsep]
  ...
  \item \textbf{What a result would mean.} Unchanged, and load-bearing: the
        ...
        Phase-1 claim.
\end{enumerate}
Separately from those four questions, the three former blockers of a
...
two-stage pipeline it provably subsumes (as the diagonal-$B$, shared-shape,
no-feature slice of its parameter space), a claim preregistered and confirmed once
...
$d_5$, $\pi(p)$ & circle-of-fifths distance; pitch-class position map \eqref{eq:tonaladj} \\
...
vibrato onset delay, and $\delta_{\zeta\zeta'}$ the Kronecker delta; our
