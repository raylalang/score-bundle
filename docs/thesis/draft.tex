...
$f_0$-derived channels ($c_i$ cents, vibrato) & Machinery built (cell masks, estimator); synthetic pilot passed; real data open \\
...
The latent field is a sum of independent components, $y = y^{(G)} + \sum_f
y^{(f)}$ (the coupled graph GP and one draw per feature kernel), and $C$ is
the sum of their covariances. Since the weight vector $K_{oo}^{-1}\tilde y_o$
in \eqref{eq:posterior} is shared, the posterior mean splits \emph{exactly} by
component,
\begin{equation}
  m \;=\; m^{(G)} + \sum_f m^{(f)}, \qquad
  m^{(\cdot)} \;=\; C^{(\cdot)}_{\cdot o}\, K_{oo}^{-1}\, \tilde y_o,
  \label{eq:decomp}
\end{equation}
and each term is itself the posterior mean $\mathbb{E}[\,y^{(\cdot)} \mid \tilde y_o\,]$
of that component --- the functional-ANOVA decomposition of an additive GP.
Observation noise shapes the shared weights but contributes no mean term of its
own at held-out notes. The posterior covariance decomposes likewise, with
cross-terms $-\,C^{(a)}_{\cdot o} K_{oo}^{-1} C^{(b)}_{o\cdot}$ between distinct
components --- negative wherever two components can explain the same variation
(explaining-away), so their normalised diagonals measure per-note redundancy
between information sources. The split is the evidence-fitted model's own
attribution --- it depends on the learned $B$, $c_f$, $\varsigma^2$ --- a
description of how the fitted model divides labour, not a causal claim; its
measured shape is reported in \S\ref{sec:attribution}.
...
over- or under-confident. A synthetic pilot (\S\ref{sec:phase2-pilot}) reverses
...
\emph{fallback} for a mis-calibrated real-world tracker, not the default.
...
A third control needs no refitting at all: the exact component decomposition of
the posterior \eqref{eq:decomp}, read off the fitted model itself
(development pieces, $\nPieces \times 4$ anchor masks). Covariance shares of the
posterior \emph{mean} at held-out notes --- summing to one per channel --- put
most of the mean on the score-feature kernel on every channel
($0.69/0.60/0.40$ for $\tau$ / $\log r$ / $v$), give the embeddings their
largest share on loudness ($0.34$, the channel where they carry signal the
...
posterior cross-correlations between components complete the picture: graph
...
$\times$ embeddings, $-0.18$ on loudness) and between graph and score features
($-0.19/-0.18$ on articulation and loudness). Both statements are the fitted
...
\figph{posterior_components_dev.png}{1.0}{The posterior mean of
\eqref{eq:posterior} split exactly by prior component \eqref{eq:decomp} on one
development piece (40\% hidden, anchor mask): coupled graph GP, score-feature
kernel, and music-model-embedding kernel sum to the grey total. The embedding
component is nearly flat on timing and articulation here and active on
loudness; the graph component carries the articulation level and its local
excursions; the score features carry most of the timing
mean.\label{fig:posterior-components}}
...
ensemble; articulation $0.546$ vs.\ $0.755$; loudness $0.066$ vs.\ $0.084$),
...
metric-replacement penalty persists throughout. Second, at the densest level
(10\% hidden) the harmonic learned-graph variant beats the plain graph
\emph{under the final model as well}: $\Delta$RMSE $-0.0089$
$[-0.0158, -0.0027]$, paired, development; NLL better on $22/30$ pieces
(median $-0.030$) with the mean contrast dominated by a single cell of the
documented Gaussian-tail piece (\S\ref{sec:guard-gp}), so no significance
claim either way on that axis. The redundancy conclusion is therefore
\emph{masking-level-scoped}: where observation is dense enough to determine
the extra edge weights, the harmonic edges carry information the embeddings
do not (consistent with the near-zero posterior correlation between the graph
and embedding components, \S\ref{sec:attribution}); the intermediate levels
are not yet measured under the final model. Whether the harmonic graph enters
the thesis model is accordingly a question for a second preregistered
confirmation set (\S\ref{sec:hygiene}), not a development decision. The
deeper lesson --- theory-as-extra-structure helps,
...
\begin{enumerate}[leftmargin=*,noitemsep]
  \item \textbf{Per-channel missingness --- resolved in code.} Short notes support
        an intonation offset but not vibrato parameters, so targets go missing per
        (note, channel). The mask interface is now generalised to per-(note, channel)
        index sets, unit-pinned exact, and validated in the synthetic pilot
        (\S\ref{sec:phase2-pilot}).
