...
\subsection{The measured constraint (development study, 2026-08)}
...
The formulation (\S\ref{sec:phase2}) is no longer future work: the
pipeline runs end to end on real recordings, the four questions this
section once posed (per-channel missingness, where alignment error goes,
the loudness channel, what a result would mean) are settled by
measurement and recorded where they belong (\S\ref{sec:phase2},
\S\ref{sec:phase2-real}), and the registered claim set has been spent
on its one-shot confirmation. What is genuinely future here is fourfold.
\textbf{Beyond URMP:} on any corpus without note-level onset
annotations the general audio-alignment problem returns, and $\tau$'s
noise row must then carry an aligner's error. \textbf{The timing
claim:} the one registered claim that failed at confirmation ---
$\tau$ calibration --- names its own follow-up: a correlated (or
heavy-tailed) $\tau$ noise row that can represent alignment error's
structure along score time, which the diagonal row provably cannot.
\textbf{The tonal metric:} the circle-of-fifths \emph{replacement}
geometry \eqref{eq:tonaladj} that hurt piano expression helps
intonation on both axes (\S\ref{sec:phase2}, development,
exploratory); adopting it channel-scoped requires its own preregistered
confirmation, and the power analysis for that design is drafted
(calibration as the primary claim --- the recovery contrast is
underpowered below ${\sim}20$ pieces). \textbf{New channels:} the
within-note drift and decay slopes measured in \S\ref{sec:phase2}
remain candidates under the $\delta^{\mathrm{vib}}$ template (adopt as
output, claim nothing) if a transcription use-case wants them.
...
now measured (\S\ref{sec:phase3-study}): exact grid inference over the
...
the same substitute geometry helps (\S\ref{sec:phase2}) --- the rule is
channel-relative.
