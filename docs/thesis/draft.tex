...
vibrato and timing, with coverage on target on every channel; the claim set
is registered and its confirmation pool untouched. Phase~3 (audio via a differentiable
...
$f_0$-derived channels ($c_i$ cents, vibrato, $\ell_i$) $+$ $\tau$, $\delta^{\mathrm{vib}}$ & \textbf{Running on real data} (URMP, development, six channels): graph value significant on intonation/vibrato recovery and vibrato/timing calibration; claims registered, confirmation pool unspent \\
...
distinct from the performed duration $\delta_i$ of \eqref{eq:logr}). Of the two decisions
...
offset chain, so the evaluated bundle there is the remaining six). The
second stays open: whether $\ell_i$,
...
tempo line's leave-one-out predictive variance --- carrying the aligner
error's scale, though not its correlation along score time, per the
limitation stated above (development evidence).
...
delay $\delta^{\mathrm{vib}}$ --- the six-channel bundle --- with estimator
variances as given, per-(note, channel) missingness
...
vibrato. The result, paired against the no-graph ablation under the
...
nominal $90\%$ on every channel of the six. Reported rather than averaged
away: under this variant the loudness and delay recovery contrasts are
small and not significant ($-0.003$, $-0.001$), the loudness NLL contrast
is $+0.04$ \emph{against} the graph, and the brass intonation cell is not
significant on its own (the vibrato win holds within every family;
the intonation win within strings and woodwind). The learned-scale variant
makes every recovery contrast significant ($-0.92$ cents, $-0.18$, $-0.24$,
$-0.006$, $-0.008$, $-0.004$) but stays the fallback, being the
worse-calibrated variant against quasi-truth (coverage $0.72$--$0.86$ vs
$0.82$--$0.86$); the ordering of systems is unchanged when scored against
the quasi-truth targets derived from the provided pitch
curves. Estimator-missing vibrato cells on visible notes
...
Phase-2 claim awaits the frozen confirmation pool under the registered
claim set (intonation recovery; vibrato calibration; coverage; timing
calibration --- the delay channel deliberately carries no claim).
...
\begin{enumerate}[leftmargin=*,noitemsep]
  ...
  \item \textbf{What a result would mean.} Unchanged, and load-bearing: the
        ...
        Phase-1 claim.
\end{enumerate}
The three former blockers of a confirmable Phase-2 result are closed:
$\tau$ is adopted into the evaluated bundle; the vibrato delay is IN by a
criterion fixed before its numbers were computed (it carries no registered
claim --- its graph contrasts are neutral); and the claim set is frozen in
the registered preregistration document. What remains is the deliberate,
