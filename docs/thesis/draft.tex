...
\newcommand{\nllConfOldHead}{$-0.311$}
\newcommand{\dRmseConf}{$-0.014$}      % paired, proposed model vs.\ strongest constrained configuration, RMSE (sig.)
...
Feat $+$ network $+$ harmonic graph (strongest constrained configuration) & \rmseHeadline{} & \nllHeadline{} & \covHead \\
...
\section{Status as of 2026-07-03: headline (two-stage plug-in baseline)}
...
significant gain on both axes on top of any mean, giving the strongest constrained
configuration (feature + network mean + harmonic graph, RMSE~\rmseHeadline{}
development). The honest framing
...
coregionalization matrix of \S\ref{sec:coreg}. Three classical profiles are used:
\begin{align}
...
pitch. \emph{Nothing in the prior changes}: the kernel \eqref{eq:gpfirst}, its
hyperparameter fit (\S\ref{sec:eb}) and the exact conjugate posterior
(\S\ref{sec:posterior}) are reused verbatim. Three things do change, and this section
...
\begin{equation}
  y_i \;=\; \bigl[\, \tau_i,\ \log r_i,\ \ell_i,\ c_i,\ f_i^{\mathrm{vib}},\ \gamma_i,\ \delta_i \,\bigr]
  \;\in\; \R^{k},
...
is the static intonation offset; and $(f_i^{\mathrm{vib}}, \gamma_i, \delta_i)$ are the
vibrato rate, extent and onset delay defined below. Two decisions are genuinely open
and should be settled before any number is quoted: whether $\delta_i$ is identifiable
...
\begin{equation}
  ...
  u_i(t) \;=\;
  \begin{cases}
    \gamma_i \sin\!\bigl(2\pi f_i^{\mathrm{vib}}\,(t - t_i - \delta_i)\bigr), & t \ge t_i + \delta_i,\\[2pt]
    0, & \text{otherwise},
  \end{cases}
  \label{eq:vibrato}
\end{equation}
...
$\delta_i$ (s) the delay before it begins. Estimating $(c_i, f_i^{\mathrm{vib}},
\gamma_i, \delta_i)$ is a small nonlinear least-squares fit per note on the voiced
...
the minimum voiced duration below which $(f_i^{\mathrm{vib}}, \gamma_i, \delta_i)$ are
...
(\S\ref{sec:gpfirst}). Per-(note, channel) missingness breaks that structure and costs
either the Kronecker factorisation or an imputation step upstream. It has no
solution in the present formulation and must be resolved before Phase 2 is built.
...
the $\tau$ row of \eqref{eq:phase2-noise}, or withhold $\tau$ from Phase-2 claims
entirely and report only the channels the audio measures directly. This is the single
...
--- no practical content. Read down the two blocks: the profile is inert, the graph is not.}
\label{tab:kernels}
\footnotesize
\begin{tabular}{@{}lcccc@{}}
\toprule
...
$k \times k$ as channels join the bundle, and the observation noise $\Sigma_e$
becomes substantive because the targets are now estimates rather than symbolic
MIDI values --- everything else, including the exact conjugate posterior, is
untouched.

\figph{arch_phase2.pdf}{1.0}{Phase 2 (not yet implemented; dashed). Audio enters
only to \emph{derive} per-note targets ($c_i$ cents, vibrato). Exactly one block
changed relative to Figure~\ref{fig:arch-phase1}: the likelihood, where
$\Sigma_e$ is now substantive. The prior branches and the conjugate posterior
...
yet started. Figure~\ref{fig:arch-phase3} shows what survives and what breaks:
...
in closed form). Again exactly one block moved relative to
Figure~\ref{fig:arch-phase1}.\label{fig:arch-phase3}}
...
prior mean, or the connection. They are not exclusive, and each is testable by the
same held-out recovery-and-calibration criterion. The first route is partly done; the
other two are open.
...
\emph{phrase} weights; a Tonnetz / neo-Riemannian lattice metric or a Tymoczko
voice-leading distance as alternatives to \eqref{eq:fifths}; and learning the
edge-family weights $w_{\mathrm{ch}}, w_{\mathrm{vl}}$ by evidence rather than fixing
them, since the reported runs use untuned defaults. Each changes $W_{ij}$, hence
$\Lg$, hence the kernel, and drops straight into the existing conjugate posterior.
