...
bars for sharper wrong ones. A 376-note development study across all three
instrument families then confirmed both halves at scale: the waveform
posterior sits a median $2.4$ cents from the quasi-truth centre with no
tracker in the loop --- beating the dedicated estimator chain on the wind
family --- and adding the drift term improves accuracy but leaves the
overconfidence exactly in place (coverage at nominal $90\%$: $1$--$3\%$),
because a better mean model shrinks the posterior width as fast as the
error. A generic noise-side fix fails as well, diagnostically: AR(1)
residual whitening (median fitted correlation $0.944$) leaves coverage
unrepaired and sharpens the overconfidence, because the structured misfit
is concentrated \emph{in band}, at the harmonics themselves, where a
stationary colored floor cannot reach. A marginalized pitch-curve deviation prior (smooth zero-mean
deviations along the curve Jacobian, treated like the amplitudes) was
measured as well: it delivers the best accuracy of all variants (median
$2.29$ cents, approaching the estimator chain's $2.01$) and leaves coverage
unmoved. The resolving measurement is a self-consistency check: on
synthetic notes where the model is true by construction, the same posterior
covers at exactly $0.90$ --- the machinery is internally calibrated, so the
invariant overconfidence against quasi-truth is an \emph{estimand} gap
(the constant the harmonic model estimates versus the constant the NLLS
estimator defines on the ground-truth curve), not an inference failure.
The design conclusion follows: the waveform posterior should enter the
...
per-note discrepancy floor at the measured $\sim 2$-cent scale ---
restoring calibration by construction under the as-given discipline the
thesis already runs on; waveform-native calibration (a bridge model for
the estimand gap) is the frontier beyond that. This measured
constraint, not the synthesis sketch, is what the phase's design now
starts from.
