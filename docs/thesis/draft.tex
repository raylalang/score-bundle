...
\otimes X_f X_f^{\top}$, and $C = \sum_a C^{(a)}$. Here $a$ indexes the
prior components, $a \in \{G\} \cup \{f\}$ --- in Phase 1,
$a \in \{G, \mathrm{feat}, \mathrm{emb}\}$ --- and $b$ below ranges over the
same set. Note the components do \emph{not} correspond to the three summands
...
cell-mask GP --- on 77 of the 78 unique development tracks (shared
recordings deduplicated; one track falls below the minimum of 30 usable
notes), hiding 30\% of notes. One target-quality rule was added with
...
\begin{equation}
  \omega^{(a)} \;=\;
  \frac{\operatorname{Cov}_{i \in \mathcal{H}}\bigl(m^{(a)}_i,\, m_i\bigr)}
       {\operatorname{Var}_{i \in \mathcal{H}}\bigl(m_i\bigr)},
  \qquad \sum_a \omega^{(a)} = 1
  \label{eq:share}
\end{equation}
...
component correlations \eqref{eq:compcorr} above --- and least often between graph and embeddings on
...
$y^{(a)}$, $C^{(a)}$, $m^{(a)}$ & prior components ($a \in \{G, \mathrm{feat}, \mathrm{emb}\}$ in Phase 1), their covariances, posterior-mean components \eqref{eq:components}--\eqref{eq:decomp} \\
$\omega^{(a)}$, $\rho^{(ab)}_i$ & covariance share \eqref{eq:share}; per-note component correlation \eqref{eq:compcorr} \\
