...
the MIDI velocity $v_i$: with the note's audio span split into four equal
chunks of samples $x^{(q)}$, $q = 1, \dots, 4$,
\begin{align}
  ...
  \nonumber
\end{align}
centred per recording because absolute level is arbitrary per track;
$s^2_{\mathrm{chunks}}$, the sample variance of the four log chunk-RMS values,
supplies its estimator noise; $c_i$
...
typically five to eight \cite{prame1994}), the extent $\gamma_i$ (how far the pitch swings
from the centre at its peak, in cents --- commonly a few tens \cite{prame1997}), and the
...
established analysis task \cite{driedger2016}. Note that $c_i$ is \emph{not} the mean of $c_i(t)$: the mean coincides with
...
gone ($-0.0037$, ns, with the collapsed piece excluded; excluding only the
collapsed cell instead reads as a small significant NLL harm, $+0.027$)
--- and that rate also produced the one fit collapse of
