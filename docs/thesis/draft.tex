...
the \emph{bundle} --- the everyday sense of the word; the title's geometric
sense, per-note fibres over the score, is the outlook of
\S\ref{sec:future-geom}. The per-note vector becomes
\begin{equation}
...
are drawn honestly. The held-out scores are unaffected: they are computed
at the randomly hidden notes, on cells the estimator could measure. But
...
itself significant: the bundle's one significant result \emph{against} the graph. In the
...
the one-shot confirmation rather than discovered by it. Estimator-missing vibrato cells on visible notes
