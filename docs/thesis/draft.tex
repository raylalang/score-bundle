...
\newcommand{\rmseNetGraph}{0.393}      % network + plain graph (development, strict)
\newcommand{\rmseStack}{0.388}         % features + network + plain graph (development, strict)
...
\newcommand{\rmseNetOff}{0.446}        % network mean, graph off (strict: logs/maskaware.log music model-ma off)
\newcommand{\nllNetOff}{$-0.164$}
...
\newcommand{\nllConfOldHead}{$-0.311$}
\newcommand{\dRmseConf}{$-0.014$}      % paired proposed model - old headline, RMSE (sig.)
...
\section{Terminology}\label{sec:twostage}
...
a plug-in estimate, and the equality is pinned by unit tests (\S\ref{sec:gpfirst}).
...
\textbf{\ + music-model embeddings as kernel (= the proposed model)} & \textbf{\rmseGPdev} & \nllGPdev & 92.7\% \\
\bottomrule
\end{tabular}
\end{table}

\noindent The development ablation sequence isolates each ingredient. Two attribution facts
matter: (i) fixing the two-stage pipeline's own mean inside the coupled GP changes
almost nothing (0.390 development) --- so the win comes from \emph{per-piece Bayesian
weighting of the features under one joint marginal likelihood}, not from coupling or joint fitting
per se; (ii) removing the graph from the full model costs $+0.017$ RMSE and
$+0.067$ NLL (development, paired, significant) --- the graph remains what keeps the
confidence honest.
...
of the claim, for the proposed model:

\begin{table}[h]\centering
\footnotesize
\begin{tabular}{@{}p{3.0cm}p{10.0cm}@{}}
\toprule
Channel & What the model does (development) \\
\midrule
...
($0.627 \to 0.601$) and, across the development ablation sequence, the channel whose
...
\begin{enumerate}[leftmargin=*,noitemsep]
  \item A reproducible pipeline: from-scratch music model (Phase~0), score-graph GMRF
...
\bottomrule
\endfoot
June & Foundations: testable thesis + music-music model backbone (Phase~0) &
Fix the task as score-informed performance transcription; choose Phase-1 targets;
build the from-scratch tokenizer + music model and run a first pretraining; inspect
...
\section{Update --- 2026-07-09/10: the reformulation}
...
\S\ref{sec:boundary}). This is the thesis model; the two-stage systems below
...
(feature + network mean + harmonic graph, RMSE~\rmseHeadline{} development). The honest framing
...
Six tasks map the reach of the structured prior; all six were \emph{re-run under the proposed model}, with the two-stage comparison given side by side in
...
\begin{table}[h]\centering
\caption{Downstream tasks and verdicts under the proposed model (re-validated).}
...
Knowing when to trust (selective prediction) & transfers & RMSE on the confident half $0.076$ \\
...
\begin{itemize}[leftmargin=*,noitemsep]
  ...
  \item \textbf{A self-checking fit.} The per-piece confidence fit validates itself
        ...
        reports the median, so no single bad evaluation cell (one piece under one mask
seed) hides in the mean.
\end{itemize}
...
\begin{enumerate}[leftmargin=*]
  \item \textbf{Headline system --- resolved twice within the week.} On 2026-07-09,
        \emph{feature + network mean + harmonic graph} (RMSE~\rmseHeadline{} development)
        was adopted as the strongest two-stage configuration, after the kernel
        comparison and a bitwise zero-leak audit. Within the day, the reformulation subsumed it (see the update at the head of this chapter):
        the preregistered confirmation made the proposed model the thesis model,
...
Prior hyperparameter identifiability (precision vs.\ smoothness trade-offs) & Fix graph geometry first, then learn precision/noise; check sensitivity. & \textbf{Hit \& mitigated} --- noise-level floor + self-validating safeguard \\
From-scratch the music model may underfit & Keep it small; verify it adds signal over the hand-built baseline before depending on it. & \textbf{Outcome:} ties features on error, adds calibration + loudness; scaling did not help \\
...
separately, now merged here as the single source
...
\section{The graph Gaussian process prior (development form)}\label{sec:graphgp}
...
graph. This section states that prior in the \emph{per-channel} form used throughout
development; the thesis model (\S\ref{sec:gpfirst}) keeps the same graph GP but
couples the channels and specifies it directly by its covariance. Here we write it
through its \emph{precision} (inverse covariance) $\Qg = K^{-1}$, in one of two forms:
\begin{align}
...
\paragraph{Precision or kernel --- a computational choice, not a modelling one.}
Writing the prior through $\Qg = K^{-1}$ rather than through $K$ does not change the
distribution, only how it is computed. In the per-channel development form $\Qg$ is
\emph{sparse} --- each note couples only to its graph neighbours, which is precisely
the Markov property of a \emph{Gaussian Markov random field} (GMRF) --- whereas the
kernel $K = \Qg^{-1}$ is dense; that sparsity is what made the per-channel posterior
(\S\ref{sec:posterior}) cheap. The thesis model makes the opposite choice for the
opposite reason: it is specified \emph{by its covariance} \eqref{eq:gpfirst} and
evaluated spectrally, $K_G = U\,g(\nu)\,U^\top$, because the coregionalization and
the feature kernels are naturally covariance-side, and a per-piece graph is small
enough that one eigendecomposition costs less than exploiting sparsity would save.
Both routes describe the same graph GP; neither is a modelling commitment.
...
behaviour), while the $\lambda I$ ridge pulls the field toward its mean and sets the
overall scale. In this development form the joint prior over the stacked field is
\emph{block-diagonal across channels}, each with its own hyperparameters and length
scales --- an assumption the thesis model drops, coupling the channels through the
matrix $B$ of \eqref{eq:gpfirst}. Throughout, $\alpha$ is \emph{only} the Mat\'ern
exponent, $\eta$ \emph{only} the additive Laplacian weight, and $\sigma_g$ the
Mat\'ern prior scale --- plain $\sigma$ is reserved for the posterior standard
deviation (\S\ref{sec:posterior}).
...
removed from the music model \emph{input}, so $\muLM$ at a held-out note depends only on the
...
\section{Observation model and posterior (development form)}
\label{sec:posterior}
This section derives the \emph{per-channel} posterior used throughout development.
The thesis model (\S\ref{sec:gpfirst}) replaces it with exact conjugate inference in
the joint, channel-coupled GP; the observation model and the masking convention
below carry over unchanged. Observations are the latent field plus independent
Gaussian noise,
\begin{equation}
...
\paragraph{The four development systems as special cases.}
The two-stage systems compared in Chapter~\ref{ch:results} are simply choices of the prior
...
\section{Hyperparameters (empirical Bayes, development form)}
\label{sec:eb}
The thesis model learns all of its hyperparameters --- the graph shape $s$, the
coregionalization $B$, the feature scales, and the per-channel noise --- jointly, by
one exact marginal likelihood per piece (\S\ref{sec:gpfirst}). The development
pipeline instead fitted each channel separately, as follows; the noise floor and the
safeguard introduced below carry over to the thesis model unchanged.
...
On a minority of mask realisations the unconstrained maximiser of \eqref{eq:marginal likelihood}
...
maximiser of \eqref{eq:marginal likelihood} lands on degenerate $(\lambda, \eta)$ that the noise
floor cannot catch --- the fitted field then extrapolates catastrophically to the
held-out nodes. The safeguard screens the fit on data it did not use: hold back a
...
back down a ablation sequence: marginal likelihood $\to$ the calibration-split objective $\to$ a
conservative decoupled fit ($\eta = 0$,
$\lambda = 1/\widehat{\operatorname{Var}}[\tilde y_o - \mu_o]$). On healthy fits the
screen never fires, so safeguarded and unsafeguarded runs are identical except at the
collapsed cells.
...
Kronecker-compatible). A safeguarded variant of the fit exists
(\S\ref{sec:guard-gp}).
...
reproduces its results almost unchanged --- which is how we know the proposed model's gain
...
music-model-mean-only baselines, on held-out, contamination-filtered data.
...
cell cannot hide in the average (the safeguard of \S\ref{sec:eb} addresses the cause).
...
Chapter~\ref{ch:exec}): on \nConfPieces{} pieces untouched by any decision, the proposed model beats the strongest two-stage pipeline on recovery
...
confirmed (paired NLL \dNllGraphConf). The development ablation sequence
...
while the \emph{median} piece is better calibrated under the GP. A safeguarded fit in
the same spirit as the two-stage safeguard (calibration-split screen including an
overconfidence check, fallback ablation sequence) was built and validated: it is bitwise
...
\section{task-matched pretraining: task-matched pretraining, an honest negative}
...
findings that motivated it --- justifies the kernel choice \emph{inside} the proposed model: the additive/regularized-Laplacian shape used in
...
weights, and the connection-Laplacian gauge construction described in the accompanying notes.
