...
with coverage on target on every channel; the claim set
...
$f_0$-derived channels ($c_i$ cents, vibrato, $\ell_i$) $+$ $\tau$, $\delta^{\mathrm{vib}}$ & \textbf{Running on real data} (URMP, development, six channels): graph value significant on intonation/vibrato recovery and vibrato/timing calibration (extent-calibration star seed-sensitive); claims registered, confirmation pool unspent \\
