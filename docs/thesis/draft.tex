...
\begin{equation}
  y \;=\; \sum_{a} y^{(a)}, \qquad
  y^{(a)} \sim \N\bigl(0,\, C^{(a)}\bigr) \text{ independent}, \qquad
  \tilde y \;=\; y + e,
  \label{eq:components}
\end{equation}
with $C^{(G)} = B \otimes K_G(s)$, $C^{(f)} = \operatorname{diag}(c_f)
\otimes X_f X_f^{\top}$, and $C = \sum_a C^{(a)}$. Because the other
components and the noise are independent of $y^{(a)}$, its cross-covariance
with the observations is just its own block,
$\operatorname{Cov}\bigl(y^{(a)},\, \tilde y_o\bigr) = C^{(a)}_{\cdot o}$,
and conditioning the jointly Gaussian pair $\bigl(y^{(a)}, \tilde y_o\bigr)$
gives each component its exact posterior:
\begin{align}
  m^{(a)} \;&:=\; \mathbb{E}\bigl[\, y^{(a)} \mid \tilde y_o \,\bigr]
  \;=\; C^{(a)}_{\cdot o}\, K_{oo}^{-1}\, \tilde y_o,
  \label{eq:decomp}\\[2pt]
  \operatorname{Cov}\bigl(y^{(a)},\, y^{(b)} \mid \tilde y_o\bigr)
  \;&=\; \delta_{ab}\, C^{(a)}
  \;-\; C^{(a)}_{\cdot o}\, K_{oo}^{-1}\, C^{(b)}_{o \cdot}.
  \label{eq:decomp-cov}
\end{align}
...
share} allocates the posterior mean's variation over the held-out notes $h$
of a piece to the components,
\begin{equation}
  s^{(a)} \;=\;
  \frac{\operatorname{Cov}_{i \in h}\bigl(m^{(a)}_i,\, m_i\bigr)}
       {\operatorname{Var}_{i \in h}\bigl(m_i\bigr)},
  \qquad \sum_a s^{(a)} = 1
  \label{eq:share}
\end{equation}
--- the normalisation is exact by bilinearity applied to
$m = \sum_a m^{(a)}$, so cross-component interactions land inside the shares
...
\begin{equation}
  \rho^{(ab)}_i \;=\;
  \frac{\operatorname{Cov}\bigl(y^{(a)}_i,\, y^{(b)}_i \mid \tilde y_o\bigr)}
       {\sigma^{(a)}_i\, \sigma^{(b)}_i},
  \qquad
  \sigma^{(a)}_i = \sqrt{\operatorname{Var}\bigl(y^{(a)}_i \mid \tilde y_o\bigr)},
  \label{eq:compcorr}
\end{equation}
...
$y^{(a)}$, $C^{(a)}$, $m^{(a)}$ & prior components, their covariances, posterior-mean components \eqref{eq:components}--\eqref{eq:decomp} \\
$s^{(a)}$, $\rho^{(ab)}_i$ & covariance share \eqref{eq:share}; per-note component correlation \eqref{eq:compcorr} \\
