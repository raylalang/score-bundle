...
\newcommand{\rmseHeadline}{0.379}      % two-stage-era headline: feat+net+chord graph (development; superseded by \rmseGP)
\newcommand{\nllHeadline}{$-0.346$}
\newcommand{\nllZero}{$-0.007$}
\newcommand{\nllGraph}{$-0.308$}
\newcommand{\nllNetGraph}{$-0.322$}
...
\newcommand{\nllConfOldHead}{$-0.311$}
\newcommand{\dRmseConf}{$-0.014$}      % paired, proposed model vs. adopted two-stage headline, RMSE (sig.)
...
\begin{abstract}
\noindent Given a printed score and a recording of a performance (as piano
MIDI in this work), we recover \emph{how} each written note was played ---
its micro-timing, articulation, and dynamics --- together with a calibrated
statement of confidence. The model is \textbf{one multi-output Gaussian
process on the score graph}: notes related in the score are related in
performance (a graph GP in the sense of Borovitskiy et al.), the expressive
channels are coupled by a coregionalization matrix, and all side
information --- hand-built score features and per-note embeddings from a
small, from-scratch symbolic music language model (the \emph{music model})
--- enters as linear kernels, i.e.\ as a marginalized Bayesian linear mean.
One exact marginal likelihood per piece learns every hyperparameter.

Development used \nPieces{} held-out ASAP pieces; the headline claims were
then \emph{preregistered and confirmed exactly once} on \nConfPieces{}
further pieces untouched by any modelling decision: the model recovers
hidden notes with lower error than the strongest \emph{two-stage} pipeline
it subsumes --- a plug-in mean followed by a graph GP on the residual
(RMSE~\rmseGP{} vs.\ \rmseConfOldHead, paired difference \dRmseConf{}
significant) --- the graph's contribution to calibration is confirmed
(paired NLL \dNllGraphConf), and nominal-90\% intervals cover
$\approx\covGP$ of the truth. Six downstream tasks map where the structure
helps (per-note recovery, confidence, error spotting) and where it does
not (whole-performance summaries; extrapolation from a short excerpt).

Phase~2 carries the same prior, unchanged, to intonation, vibrato,
loudness, timing, and vibrato onset delay on continuously pitched
instruments (URMP): the full estimator pipeline runs end to end on a
six-channel bundle with heteroscedastic per-(note, channel) observation
noise, and the registered claim set was \emph{confirmed in a single
one-shot} on the frozen 13-piece pool --- intonation recovery, vibrato
calibration on both channels, coverage on target on all six channels; the
secondary timing-calibration claim failed and is reported verbatim.
Phase~3 (audio via a differentiable synthesizer) is scoped, with a first
development study measured: the waveform alone localizes intonation to
within about two cents of the ground-truth-derived centre, and the study
fixes the phase's central design constraint --- calibration must bridge
the estimand gap.
\end{abstract}
...
The title names the object of study. With each note of the known score we
associate a small \emph{bundle} of expressive variables --- timing,
articulation, dynamics; later intonation, vibrato, loudness --- so a
performance is a per-note field over the score support, and transcription
is posterior inference over that field. ``Bundle'' is used in this
everyday sense throughout; the geometric reading (per-note fibres with a
connection over the score graph) is a deliberate outlook reserved for
future work (\S\ref{sec:future-geom}).
...
Two names recur throughout. \textbf{The proposed model} is the thesis model:
one multi-output Gaussian process over all notes and channels, in which the
score graph, the hand-built score features, and the per-note representation of
the music model (the small from-scratch symbolic language model of Phase 0)
enter a single kernel, every hyperparameter learned from one per-piece
marginal likelihood (\S\ref{sec:gpfirst}). \textbf{The two-stage pipeline} is
the earlier development system it provably contains as a special case
(\S\ref{sec:special-cases}): a \emph{plug-in mean} --- a cross-piece point
estimate of each note's expression --- followed by a separate per-channel
graph GP on the residual.
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{Methodological rigour as a result in its own right}: a discovered
        ...
        (\S\ref{sec:hygiene}).
\end{enumerate}
A path to audio via a differentiable-synthesizer likelihood (Phase~3) and
a geometric ``connection-Laplacian'' generalisation of the graph prior are
deliberately \emph{not} claimed as contributions: both are future work
(Chapter~\ref{ch:future}), the former with a first development study
already measured (Appendix~\ref{app:phase3}).
...
The work is phased so that each stage produces a defensible result before the
next risk is taken; Table~\ref{tab:phases} records the phases and their
status.
...
$f_0$-derived channels ($c_i$ cents, vibrato, $\ell_i$) $+$ $\tau$, $d^{\mathrm{vib}}$ & \textbf{Confirmed} (URMP, registered one-shot: intonation recovery, vibrato calibration, coverage; the secondary timing claim failed, reported verbatim --- Chapter~\ref{ch:phase2}) \\
3 --- extension (waveform) & Differentiable synthesizer as audio likelihood &
Harmonic amplitudes $a$, audio $x$ & Scoped; first waveform study measured (development, exploratory --- Appendix~\ref{app:phase3}) \\
\bottomrule
\end{tabular}
\end{table}
...
synthesis. Chapter~\ref{ch:model} gives the Phase-1 model and derivation.
...
Chapter~\ref{ch:phase2} carries the same prior, unchanged, to real recordings and
reports the Phase-2 confirmation.
Chapters~\ref{ch:discussion}--\ref{ch:conclusion} discuss, situate, and conclude;
appendices hold the notation summary (\ref{app:notation}), the two-stage
development lineage (\ref{app:lineage}), the methodology record
(\ref{app:methods}), the relocated Phase-1 and Phase-2 development studies
(\ref{app:phase1dev}, \ref{app:phase2dev}), the Phase-3 mathematics and
study (\ref{app:phase3}), and additional tables (\ref{app:extra}).
...
(Appendix~\ref{app:phase3}).

A complementary line puts the periodicity itself into a Gaussian-process
\emph{kernel} rather than a deterministic basis. Spectral mixture
kernels learn a stationary covariance as a Gaussian mixture over
frequencies \cite{wilson2013}; their non-stationary generalization ---
the \emph{generalized spectral mixture} kernel --- lets the frequencies,
amplitudes, and coherence lengths vary with the input, each modeled as a
latent function \cite{remes2017}. Applied to music audio, per-note
change-windows gating GPs with harmonic (exponentiated-cosine)
covariances support pitch estimation and gap imputation directly on the
waveform \cite{alvarado2016}. These families are the natural priors for
the \emph{within-note} structure this thesis measures but does not
model --- a quasi-periodic vibrato whose rate and extent drift. Both
slots where they could enter --- a curve-level estimator and the Phase-3
pitch-curve prior --- were measured on development data with negative
verdicts (\S\ref{ch:future}); their imprint on this thesis is
conceptual. The design
difference is deliberate and worth stating: kernel-side constructions
marginalize the note's structure away, while this thesis keeps named
per-note variables ($c$, $\gamma$, $f^{\mathrm{vib}}$, \dots) because
the variables themselves are the object of study.

% =====================================================================
\chapter{Model and Formulation}\label{ch:model}
\emph{This chapter gives the complete Phase-1 model derivation; Phase 2's
delta is specified with its results (Chapter~\ref{ch:phase2}), and the
Phase-3 mathematics in Appendix~\ref{app:phase3}. Notation is summarised
...
explicitly with a leave-one-out warp, Chapter~\ref{ch:phase2}). Conditional on $T$, the three
channels are
\begin{align}
...
\caption{Performance variables by phase (Phases 1--2), with what they are
realistic for. Phase-3 quantities --- harmonic amplitudes $\mathbf a_i$,
amplitude envelope $A_i(t)$ --- are defined in Appendix~\ref{app:phase3}.}
\label{tab:variables}\\
\toprule
...
Vibrato (rate, extent, delay) & $f_i^{\mathrm{vib}}, \gamma_i, d_i^{\mathrm{vib}}$ & Hz, cents, s & 2 & voice, strings, winds \\
...
\paragraph{Naming.}
We call the graph augmented by \eqref{eq:harmadj} the \emph{chord graph}, its
two families the \emph{chord} and \emph{voice-leading} edges --- edges
\emph{of harmony}: which notes sound together, and which move by step. The
name deliberately avoids ``harmonic'', which this thesis reserves for the
\emph{harmonic amplitudes} $a$ of the Phase-3 synthesiser
(Appendix~\ref{app:phase3}), the overtones of a single note's fundamental;
the two senses never interact.
...
constraint switches of \S\ref{sec:special-cases}; the attribution experiments
locate most of the proposed model's accuracy gain precisely at this switch
...
components --- the coupled graph GP and one draw per feature kernel --- so
this posterior decomposes \emph{exactly} by prior component. The
derivation is given at its single point of use, the head of the
attribution analysis (\S\ref{sec:attribution}).
...
$(\Sigma_e)_{ii}$ in the per-channel form of Appendix~\ref{app:lineage}):
\begin{equation}
...
Its per-channel precision-side form ($\Qg = \lambda I + \eta \Lg$ per
channel, the sparse-GMRF computational route), the plug-in-mean choices,
and the constrained systems' posterior formulas are recorded in
Appendix~\ref{app:lineage}.

\paragraph{Exact nesting.}
Constraining $B$ diagonal, dropping the feature kernels, and fixing the graph
recovers the two-stage pipeline's per-channel model
(Appendix~\ref{app:lineage}) exactly \emph{on the shared-shape slice}:
...
space, which is what makes the lineage tables of Appendix~\ref{app:lineage} an
...
\section{Phase-2 corpus (URMP)}\label{sec:urmp}
...
data-blind (13 of 44 pieces in the confirmation pool, spent exactly once, in Chapter~\ref{ch:phase2}) --- and this
...
the same estimator run on the provided ground-truth pitch curves, hence
independent of the tracker though not of the estimator --- used as
a cross-check in Chapter~\ref{ch:phase2}). The claim available in
...
The published operating point hides \maskRate{} of notes --- inherited,
not derived. A post-confirmation development sweep, from 50\% hidden down
to the leave-one-out limit (Appendix~\ref{app:masksweep}), finds the model
ordering stable at every level, the graph's paired contribution
significant from 50\% down to 20\% and at leave-one-out, and coverage
insensitive throughout: the operating point is arbitrary, but not lucky.
...
(Table~\ref{tab:headline-conf}). The proposed model (\S\ref{sec:twostage}) beats the adopted two-stage
headline on accuracy (paired per-piece \dRmseConf, significant), its intervals
...
The full development lineage --- the nested ablation sequence from
zero-mean graph-off up to the proposed model (development RMSE \rmseGPdev,
NLL \nllGPdev), with the two-stage pipeline rows as its special cases, and
the three-step account of how the final model was selected --- is recorded
in Appendix~\ref{app:lineage}; development numbers flatter every system,
which is why the confirmation table above is the headline. The attribution
controls of \S\ref{sec:attribution} trace the differences to their
mechanism.
...
two-stage headline with chord edges remains significantly ahead of this
...
The pooled headline averages three normalised
...
($\nPieces \times 4$ masks), the proposed model's own ablation sequence;
the two-stage lineage rows are in Appendix~\ref{app:lineage}.}
...
\section{Target leakage: found, fixed, pinned}\label{sec:leak}
An earlier read-out let the music model see a note's own velocity --- the
very target it was asked to predict --- inflating its apparent advantage;
the read-out was moved one position earlier, everything re-measured, and
the strict protocol adopted. The corrected velocity edge is smaller but
honest (mean-only error $0.090$ against the retracted $0.057$), a
structural test makes the leak impossible to reintroduce silently, and
the proposed model's pipeline is additionally audited bitwise end to end.
The full episode --- including the scaling illusion the leak manufactured
and the one documented $O(1/N)$ centring dependence that sits upstream of
the audit's scope --- is recorded in Appendix~\ref{app:methods}.
...
(Appendix~\ref{app:lineage}), and a fixed fallback sequence --- was built and
...
post hoc: the principled fix is a heavy-tailed (Student-$t$) timing
likelihood --- since measured to defuse exactly this tail, at deploy time
and at inference level, while predictive floors are measured ineffective
(\S\ref{ch:future}) --- and the per-piece median NLL
--- reported throughout --- is robust to it by construction.

\section{Development studies, one verdict each}\label{sec:devstudies}
Four development studies scope the result; each is recorded in full in
Appendix~\ref{app:phase1dev}. \textbf{Hand-built features:} against the
25-feature score baseline the music model ties on average error; its
genuine, significant edge is calibration and the velocity channel, and
the two stack. \textbf{Calibrated deep baselines:} a heteroscedastic MLP
head and a five-member deep ensemble, run under the identical protocol
and information set, lose to the proposed model on both axes by wide
significant paired margins (RMSE $-0.090^{*}$, NLL $-0.30^{*}$) ---
ranking \emph{which} notes are uncertain requires per-piece adaptation
and the graph. \textbf{Task-matched pretraining:} a bidirectional,
mask-aware retraining at matched compute is an honest negative (pooled
error $+0.011$ worse); the same leak pattern tried to re-enter through
the new read-out and was caught by its signature. \textbf{Kernel
comparison (run under the two-stage baseline):} the classical spectral
profiles all tie the additive default; structure beyond time adjacency is
what buys calibration; music theory helps as \emph{added} chord and
voice-leading edges and hurts as a \emph{replacement} pitch metric ---
and under the final model the added edges are measured-redundant at the
operating point, with a clean density gradient across masking levels and
no second confirmation set spent.

% =====================================================================
\chapter{Downstream Tasks}\label{ch:downstream}
Six tasks with objective metrics and honest baselines; all outcomes reported
(summary in Table~\ref{tab:downstream}; both verdict columns in
Appendix~\ref{app:lineage}). Every task was run under the
...
\begin{table}[h]\centering
\caption{Downstream tasks: verdicts under the proposed model. The
two-stage development verdicts are recorded in Appendix~\ref{app:lineage}.}
\label{tab:downstream}
\footnotesize
\begin{tabular}{@{}p{4.6cm}p{8.2cm}@{}}
\toprule
Task & Verdict (proposed model) \\
\midrule
Error spotting (anomaly) & \textbf{clear win}: best on all channels ($v$ 0.995) \\
Denoising, known noise & win (0.182/0.276, cov 0.90) \\
Denoising, blind noise & failure in both parameterizations \\
Completion from excerpt & partial $+$ \textbf{extrapolation boundary} (\S\ref{sec:boundary}) \\
Selective prediction & qualified win, transfers (confident-half RMSE 0.076) \\
Era classification & honest negative (0.20 vs 0.60 majority, n=30) \\
Performer-ID (Vienna) & mixed (raw 0.136 $>$ GP-denoised 0.114) \\
\bottomrule
\end{tabular}
\end{table}
...
% =====================================================================
\chapter{Phase-2 Results: Intonation and Vibrato on Real Recordings}
...
(\S\ref{sec:eb}) and the exact conjugate posterior (\S\ref{sec:gpfirst}) are
...
\section{The model delta: the bundle, the targets, the noise}
...
the \emph{bundle}, in the everyday sense fixed in the Introduction. It becomes
\begin{equation}
...
centre sits; and $(f_i^{\mathrm{vib}}, \gamma_i, d_i^{\mathrm{vib}})$ are the
...
$18$\,ms --- so $d_i^{\mathrm{vib}}$ is a channel and $k = 7$ as
written. On the evaluation corpus (\S\ref{sec:urmp}), articulation has no measured
offset chain --- the sequence of annotated \emph{performed note ends}
that $\log r_i$ would require --- so the bundle evaluated there is the
remaining six. The
...
\paragraph{Estimates, not annotations.}
What these quantities mean to a listener --- sharp or flat and by how
...
Appendix~\ref{app:glossary}. What matters structurally: none of these
values is annotated by hand. Every one is \emph{estimated} from the
recording --- a pitch tracker supplies the note's pitch curve, and the
per-note fit below turns that curve into these numbers, each with its own
uncertainty, which is exactly why the observation noise of
\eqref{eq:phase2-noise} becomes substantial in Phase~2. The corpus's
human annotations supply only the note spans (onsets, durations, written
pitches); its ground-truth pitch curves are used to calibrate the
...
$d_i^{\mathrm{vib}}$ (s) the delay before it begins. Estimating $(c_i, f_i^{\mathrm{vib}},
\gamma_i, d_i^{\mathrm{vib}})$ is a small nonlinear least-squares fit per note on the voiced
...
$t_i + d_i^{\mathrm{vib}}$, supplies the delay itself, with its own
...
development measurements of \S\ref{sec:urmp}: note $i$'s sample set is
\begin{equation}
  J_i \;=\; \bigl\{\, \text{frames } j:\; t_j \in [t_i,\, t_i + \delta_i),\;
  \text{voiced}_j,\; \pi_j \ge q_{0.2}(\text{track}) \,\bigr\},
  \label{eq:framerule}
\end{equation}
--- the tracker's per-frame confidence $\pi_j$ thresholded at the track's
...
per-cell observation pattern the \emph{cell mask}. The conjugate posterior of \S\ref{sec:gpfirst} assumes \emph{per-note}
...
\paragraph{What the per-note compression discards (development study).}
The channels of \eqref{eq:phase2-channels} hold every parameter fixed
within the note: they are \emph{time-averages}. A development drift study
...
centre --- confirmed by the tracked and ground-truth curves
independently, median $10.5$ cents per note, an order of magnitude above
the cell's reported standard error --- yet the discarded structure is
\emph{graph-white}: within-note slopes carry essentially no note-to-note
...
right level for \emph{this} model --- the mismatch is priced into the
residual-based cell variances, which is why calibration survived at
confirmation --- while the discarded structure itself belongs to
Phase~3's frame-level likelihood, whose amplitude envelope $A_i(t)$ and
pitch-curve model are built to carry it; slope channels remain
post-confirmation candidates under the $d^{\mathrm{vib}}$ template
(included as output, claiming nothing).
...
DTW); the two tracks without a usable warp remain in the evaluation with
their $\tau$ cells missing, not dropped. The feasibility statistic uses a local leave-one-out warp: with
...
design, the hypothesis committed before the numbers;
Appendix~\ref{app:tonal}): the circle-of-fifths metric improves held-out
intonation significantly on both axes while re-imposing the familiar
replacement penalty on timing --- the tonal geometry helps exactly where
the target is pitch and hurts where it is not, the first evidence in this
thesis that a music-theoretic \emph{geometry}, rather than merely
music-theoretic \emph{edges}, earns its place. Adoption into the bundle
would be a channel-dependent metric choice and needs its own
preregistered confirmation; nothing in the registered claim set depends
on it.
...
tracker calibration, the composition-level development/confirmation split,
and the sense in which ``recovery'' means agreement with the estimator's
targets rather than with a gold standard. That sense applies to every
...
Before any real audio, the full specified pipeline was validated end to
end on synthetic pieces whose per-note truth is known
(Appendix~\ref{app:pilot}): the graph GP beats the no-graph ablation on
every channel; missing vibrato cells --- which no per-note method can
even express --- are recovered at roughly $4\times$ better than prior
scale with honest coverage; and the as-given noise default is settled
there. A synthetic result: it validates machinery and settles design
defaults, and claims nothing about real audio.
...
delay $d^{\mathrm{vib}}$ --- the six-channel bundle. Estimator variances
...
\section{The one-shot confirmation (run once, 2026-08-27)}
...
Figure~\ref{fig:arch-phase2} records the architectural claim: relative
...
not carried over), and the observation noise $\Sigma_e$ becomes
...
The attribution split (\S\ref{sec:attribution}) --- per-piece feature
weighting recovers, the graph calibrates --- invites two questions the
numbers alone do not answer. The posterior
...
\section{Limitations}
Two results are \emph{confirmed}, each by its own preregistered
one-shot: the symbolic piano result (Phase 1) and the real-audio
six-channel result (Phase 2) --- the latter on a single corpus (URMP, 13
confirmation pieces), with the secondary timing-calibration claim failed
and a small starred adverse timing-recovery cell, both reported verbatim.
Phase-3 evidence is development-level and exploratory throughout.
...
failing to replicate). The remaining open threat is external validity beyond the two
studied corpora (ASAP piano; URMP chamber ensembles).

% =====================================================================
\chapter{Future Work}\label{ch:future}

\section{Model-level refinements: measured needs, now measured answers}
Three refinements were named here as needs following from measured
limitations; all three have since been measured on development data, and
two of the original recommendations turned out to be wrong (all
exploratory, published tables unchanged). A \textbf{heavy-tailed
(Student-$t$) observation model for timing} works at both levels. As a
deploy-time predictive (variance-matched, $\nu = 5$) it defuses the
demonstrated NLL tail of \S\ref{sec:guard-gp} completely --- the
documented blow-up cell falls from $105.2$ to $-1.7$ with coverage intact
--- at zero change to the fit. As an inference-level noise model (a
Gaussian scale mixture fit by leave-one-out EM reweighting) it
additionally brings timing coverage to nominal and improves
\emph{articulation} recovery ($-0.018$, significant): an observed timing
outlier corrupts the coupled channels through the coregionalization, and
only robustifying the fit undoes that. Its fits must be scored with the
matched $t$ predictive --- a Gaussian score of a $t$ fit is incoherent.
\textbf{Deploy-time predictive floors}, previously suggested here as the
cheap engineering alternative, are measured ineffective: the failure is a
tail-shape problem, not a scale problem, and a floor cannot reach
outliers whose predictive scale is already honest. The
\textbf{extrapolation-safe completion mode} is closed by a measured
combination: the Mahalanobis coverage test suggested here (workable only
in the observed excerpt's low-rank frame) is half the answer --- silent
on interpolation, as required, but blind to catastrophic notes lying
inside the excerpt's feature coverage --- and a $3\sigma$ disagreement
guard against the cross-piece head is the other half, taming every
blow-up (worst cell $480{,}266 \to 0.7$) while keeping the per-piece
model's advantage where adaptation wins; blow-ups also occur under
interpolation masks, so the boundary is about feature-weight
determination, not extrapolation alone. All three remedies are
implemented as opt-in capabilities under the safeguard precedent
(published paths bit-unchanged); adopting any into a reported protocol
requires its own registration.

\section{Phase 2: what remains}
Phase 2 itself is now a result (Chapter~\ref{ch:phase2}); what remains is
fourfold. \textbf{Beyond URMP:} on any corpus without note-level onset
annotations the general audio-alignment problem returns, and $\tau$'s
noise row must then carry an aligner's error. \textbf{The timing claim:}
the one registered claim that failed at confirmation --- $\tau$
calibration --- named its own follow-up, a correlated $\tau$ noise row
representing alignment error's structure along score time, and that
follow-up is now measured (development, exploratory): an AR(1) block over
note order, its one parameter chosen by the evidence, improves timing NLL
significantly ($-0.111$), moves coverage toward nominal, and cuts timing
RMSE by ${\sim}20\%$ ($\approx 99 \to 80$\,ms), with the correlation
detected on 97--100\% of cells across three independent runs (fresh mask
seeds; joint refit). Turning this into a claim requires its own
preregistered confirmation on a fresh pool --- the pending corpus
decision. \textbf{The tonal
metric:} the circle-of-fifths \emph{replacement} geometry
\eqref{eq:tonaladj} that hurt piano expression helps intonation on both
axes (\S\ref{sec:phase2-delta}; development, exploratory); adopting it
channel-scoped requires its own preregistered confirmation, and the power
analysis for that design is drafted (calibration as the primary claim ---
the recovery contrast is underpowered below ${\sim}20$ pieces).
\textbf{New channels:} the within-note drift and decay slopes measured in
\S\ref{sec:phase2-delta} remain candidates under the
$d^{\mathrm{vib}}$ template (adopt as output, claim nothing) if a
transcription use-case wants them.

\section{Phase 3: a differentiable-synthesizer audio likelihood}
\label{sec:future-phase3}The intended step to audio: $x = \Phi(z)\,a + \varepsilon$
(Appendix~\ref{app:phase3}), with conditionally linear amplitudes marginalised
...
implemented and unit-tested --- and a first slice of the $z$-inference is
now measured (Appendix~\ref{app:phase3}): exact grid inference over the
intonation coordinate of $z$ on real notes, and the waveform posterior's
integration into the Phase-2 bundle. The \emph{joint} loop --- all notes
and coordinates at once under the graph prior (Laplace/VI), with the
estimand bridge and the amplitude envelope $A_i(t)$ under the prior ---
is the open work. The reviewed spectral-kernel families
(\cite{wilson2013,remes2017,alvarado2016}; supervisor-suggested) were
measured at exactly this slot (development, exploratory): swapping the
deviation prior of \S\ref{sec:phase3-study} for a matched-rank
spectral-mixture prior adds nothing --- median tie ($2.25$ vs $2.29$
cents), paired mean significantly against ($+0.11$), the harm
concentrated where the scaffold rate is unreliable, since a vibrato band
pinned at a wrong rate is worse than frequency-neutral bumps. The bump
basis therefore stays; the families' measured contribution to this
thesis is conceptual (curve-level scoring, the coherence bound, the
realized-quantity read-out rule), and a revival would marginalize the
band location rather than pin it. Figure~\ref{fig:arch-phase3} shows what survives and what breaks:
...
\figph{arch_phase3.pdf}{1.0}{Phase 3 (dashed: the joint system is not yet implemented; the likelihood block is the part exercised in \S\ref{sec:phase3-study}). The
...
lesson is worth stating in general form --- with the qualifier Phase~2
measured: music theory earns its place in the kernel as \emph{extra
relations}, not as a substitute geometry, \emph{for channels whose
target is not pitch}; on intonation, where the target is pitch itself,
the same substitute geometry helps (Chapter~\ref{ch:phase2}) --- the rule is
channel-relative.
...
Fourteen music-theoretic score columns (local key, metrical weight,
dissonance, phrase salience, motif repetition, tessitura) change nothing
significant on any channel at any masking level, and a linear plus
random-features probe of the embeddings rules out the obvious
explanation: the embeddings encode voicing, rhythm, and register but
\emph{not} tonality --- scale degree, mode, and local key sit at chance.
The consistent reading, taken with the tonal-metric result of route (A):
tonal structure carries no measurable marginal signal for these three
expressive channels at this scale, while the rhythmic and voicing
structure the columns add is already in the kernel (full study:
Appendix~\ref{app:theoryfeat}). The columns remain in the codebase as an
interpretable probe, off by default.
...
The validated Phase-1 contribution stands on its own; the same prior has
now been carried, unchanged in form, to intonation, vibrato, loudness,
timing, and vibrato onset delay on real recordings --- and confirmed at
one-shot level on the frozen pool (intonation recovery, vibrato
calibration, coverage; the secondary timing claim failed and is reported
verbatim). The step to audio through a differentiable synthesizer is
where the most novel work still lies; its first development study
already fixes the design constraint the phase must solve.
...
$\ell_i$, $f_i^{\mathrm{vib}}$, $\gamma_i$, $d_i^{\mathrm{vib}}$ & Phase-2 loudness (log RMS); vibrato rate, extent, onset delay \\
...
$x$, $z$, $a$, $\Phi(z)$, $\epsilon$, $K_n$, $H$ & audio, nonlinear positions, amplitudes, synth basis, noise, noise cov, harmonic count (Phase 3) \\
...
observation precision; $\pi(p)$ is the circle-of-fifths position map while
$\pi_j$ is a tracker confidence; $B$ is the
coregionalization matrix ($B = 2000$ appears only as the bootstrap replicate
count); $\delta_i$ is the performed duration and $\delta_{\zeta\zeta'}$ the
Kronecker delta (the vibrato onset delay is $d_i^{\mathrm{vib}}$);
...
The proposed model was reached through a \emph{two-stage} development
system --- a plug-in mean plus a per-channel graph GP on the residual ---
which the final model provably contains as the all-switches-on corner of
its constraint space (\S\ref{sec:special-cases}). This appendix records
that lineage: the per-channel precision-side formulation, the constrained
systems' posteriors, the development ablation sequence, the selection
history, and the two-stage verdicts of the downstream tasks. Everything
here is development-level record; the main line carries the confirmation
comparison (Table~\ref{tab:headline-conf}).
...
\section{The constrained systems, by their switches}
...
\section{The development ablation sequence}\label{app:devabl}
...
Features $+$ music-model $+$ chord graph (adopted headline) & \rmseHeadline{} & \nllHeadline{} & \covHead \\
...
Per channel, the two-stage rows sit at the head of the same lineage
(cf.\ Table~\ref{tab:perchannel}):

\begin{table}[h]\centering
\caption{Per-channel two-stage lineage rows (development set), the rows
formerly beneath the line of Table~\ref{tab:perchannel}.}
\label{tab:perchannel-lineage}
\footnotesize
\begin{tabular}{@{}lcccccc@{}}
\toprule
 & \multicolumn{2}{c}{Timing $\tau$} & \multicolumn{2}{c}{Articulation $\log r$} & \multicolumn{2}{c}{Velocity $v$} \\
...
\section{How the final model was selected}\label{app:selection}
...
(\S\ref{sec:kernels}), \emph{features + music-model mean + chord graph}
(RMSE~\rmseHeadline{} development) was the adopted headline --- the only variant with every ingredient
...
\section{Downstream verdicts under the two-stage system}
Every downstream task of Chapter~\ref{ch:downstream} was first run under
the two-stage pipeline during development; Table~\ref{tab:downstream-both}
gives both verdict columns side by side.
...
% =====================================================================
\chapter{Methodology record: the leakage episode}\label{app:methods}
Relocated from the Phase-1 results chapter; the main line carries the
verdict (\S\ref{sec:leak}).
...
% =====================================================================
\chapter{Phase-1 development studies}\label{app:phase1dev}
Relocated development studies; each has a one-verdict summary in the
main line (\S\ref{sec:masksweep}, \S\ref{sec:devstudies},
\S\ref{sec:musictheory}). Everything here is development-set evidence.
...
survives every classical alternative profile}: Mat\'ern at
$\alpha = 2$ and $3$ (at $\alpha = 1$ the profile \emph{is} the additive baseline
...
plateau). The chord graph with the plain music-model mean (chord+VL, RMSE $0.3852$)
even edges out
the feature-stacked candidate on the plain graph (\rmseStack) --- and the two upgrades
\emph{compose}: on the feat+music-model mean the chord graphs still gain, independently
...
knife-edge. \textbf{Under the final model, the decision first:} the thesis model
keeps the plain graph at its operating point; the chord edges stand as
a measured dense-observation refinement, and any future adoption would be
a question for a second preregistered confirmation set
(\S\ref{sec:hygiene}), not a development decision. The evidence: once the
music-model embeddings enter the kernel as features, the chord edge
families are measured-redundant \emph{at the 40\% operating point} (learned-graph and
chord variants statistically tie the plain graph, development). Two
...
extreme, fragile. The deeper lesson --- theory-as-extra-structure helps,
...
($-0.004$ RMSE, paired) --- the same pattern as the chord edges: where the
...
route (A) of \S\ref{sec:musictheory}, the consistent reading is that \textbf{tonal structure carries no
...
% =====================================================================
\chapter{Phase-2 development studies}\label{app:phase2dev}
Relocated from Chapter~\ref{ch:phase2}, where each study keeps a
one-verdict summary. All of it is development-level evidence; the
registered claim set never depended on any of it.
...
here, and $\approx 71$ cents on average in professional singing \cite{prame1997}), and the
onset delay $d_i^{\mathrm{vib}}$ (how long after the note begins the
...
\section{The synthetic end-to-end pilot}\label{app:pilot}
...
\section{The drift study: what the per-note compression discards}\label{app:drift}
Eqs.~\eqref{eq:loudness}--\eqref{eq:vibrato} hold every parameter fixed
within the note: the channels are \emph{time-averages}, and it is fair to
ask what the averaging discards. The study answering this is simple. Every
vibrato-identifiable development note that both pitch curves can measure
($n = 5{,}145$) is refit with the eq:vibrato model plus \emph{one} added
ingredient --- a linear drift of the centre, $c + s\,(t - \bar t)$ under
the vibrato --- and the fit is run twice per note, \emph{independently} on
the tracked curve and on the ground-truth curve. The two-curve design is
the control: a slope appearing in one curve only would be measurement
noise; a slope two independent measurements agree on is the player.
Loudness gets the same check through the four chunk levels of
\eqref{eq:loudness}. Table~\ref{tab:drift-study} collects the numbers;
Figure~\ref{fig:drift-study} shows the argument.

\begin{table}[t]\centering
\caption{The drift study: within-note time variation, measured
(development set). Intonation slopes are fit independently on the tracked
and ground-truth curves; loudness slopes on the four chunk levels of
\eqref{eq:loudness}. The last row is the across-note test that decides
whether the discarded structure is one a graph over notes could use.}
...
\begin{figure}[t]
  ...
  \caption{The drift study in four panels (development set).
  \textbf{A}: a $2.7$-second violin note --- the constant-centre sine
  (dashed) holds the centre flat; the drift term reveals the centre
  climbing at $+17$ cents/s under the vibrato. \textbf{B}: on notes where
  both curves fit a significant slope, the tracker's slope against the
  ground-truth curve's --- two independent witnesses agreeing on
  direction $97\%$ of the time. \textbf{C}: note $i$'s drift slope
  against note $i{+}1$'s (per-track standardized, pooled): no across-note
  structure. \textbf{D}: the same plot for the timing residual $\tau$ ---
  the across-note correlation the graph prior actually uses. The
  structure the sine model discards (A--B) is real but lives only within
  notes (C); the graph operates across notes (D).}
  \label{fig:drift-study}
\end{figure}

The findings, in the order they matter. \emph{The drift is real music,
not tracker noise}: about two thirds of notes carry a significant slope
in each curve independently, and the two curves agree on its direction
$97\%$ of the time (Fig.~\ref{fig:drift-study}B). \emph{It is large
relative to the reported precision}: the median note drifts $10.5$ cents
--- an order of magnitude above the $c$ cell's median reported standard
error of $0.9$ cents --- so $c$ is a precise estimate of the note's
\emph{average} intonation, not of a constant
(Fig.~\ref{fig:drift-study}A). Loudness moves even more, though the
falling shares in Table~\ref{tab:drift-study} say most of it is decay
envelope rather than expressive shaping. \emph{And the discarded
structure is graph-white}: the within-note slopes carry essentially no
correlation from one note to the next, where timing --- the channel the
graph demonstrably helps --- carries $+0.59$
(Fig.~\ref{fig:drift-study}C--D). Three consequences keep the per-note
resolution defensible rather than refuted. The mismatch is priced in:
the cell variances of \eqref{eq:phase2-noise} are residual-based, so a
drifting note reports itself as more uncertain --- which is why
calibration survived at confirmation. The quasi-truth targets pass
through the same parameterization, so the graph-vs-no-graph contrasts
compare like with like. And a within-note slope channel is precisely the
kind the graph prior could not help --- so the per-note resolution is
empirically the right level for \emph{this} model, while the discarded
structure itself belongs to the frame-level likelihood of Phase~3, whose
amplitude envelope $A_i(t)$ and pitch-curve model are built to carry it;
slope channels remain post-confirmation candidates under the
$d^{\mathrm{vib}}$ template (included as output, claiming
nothing).

\section{The tonal-metric exploratory test on intonation}\label{app:tonal}
The hypothesis (\S\ref{sec:phase2-delta}) was committed before the
numbers; the test is exploratory by the registered design. Replacing the
semitone metric by the circle-of-fifths metric \emph{improves held-out
intonation on both axes} --- paired tonal-minus-plain under the default
noise variant, $\Delta$RMSE $-0.21$ cents $[-0.34, -0.09]$, $\Delta$NLL
$-0.05$ $[-0.07, -0.03]$, both significant --- while re-imposing the
familiar replacement penalty on timing ($\Delta$NLL $+0.03$ against,
significant); the vibrato-rate channel picks up a small significant
recovery gain ($-0.027$) and the remaining channels are indifferent
...
\chapter{Phase 3: the waveform likelihood}\label{app:phase3}\label{sec:phase3-math}
\emph{Relocated development material: the Phase-3 mathematics and its measured
development study. Main-line summary: \S\ref{sec:future-phase3}.}
...
intonation, vibrato), $\Phi(z) \in \R^{M \times 2HN}$ is the deterministic harmonic
...
$\phi_k(t) = 2\pi k \int_0^t f_0(s)\, ds$ determined by $z$ ($H$ counts
harmonics) --- and
$a \in \R^{2HN}$ are the stacked per-note harmonic amplitudes. Per-note amplitudes
...
\begin{align}
  ...
  \label{eq:collapsed}
\end{align}
writing $\Phi = \Phi(z)$, with $\mu_a \in \R^{2HN}$ and
$\Sigma_a \in \R^{2HN \times 2HN}$ for $H$ harmonics (two quadrature columns
...
\section{The measured constraint (development study, 2026-08)}
\label{sec:phase3-study}

The collapsed likelihood \eqref{eq:collapsed} has now met real audio, in a
development study built to answer two questions: does the waveform carry
the per-note expressive information directly, with no tracker in the loop
--- and is the resulting posterior honest?

\paragraph{Design.} One observation is one note's audio segment: $m$
samples at $16$\,kHz (up to two seconds; $m \approx 10^{3.5}$--$10^{4.5}$),
cut at the annotated note boundaries of a development URMP track. The
study covers $376$ notes from seven tracks spanning all three instrument
families. Per note, the $f_0$ curve is parameterized by the Phase-2
channel variables (eqs.~\eqref{eq:cents-curve}--\eqref{eq:vibrato}; on
vibrato-identifiable notes the extent/rate/delay scaffold is taken from
the Phase-2 estimator --- the one estimator-supplied ingredient, recorded
as such), and the design matrix is the harmonic basis of
\eqref{eq:waveform} with amplitudes piecewise-constant over the four
chunks of \eqref{eq:loudness}: $p = 2H \times 4 = 64$ columns at $H=8$
harmonics. Amplitudes are marginalized exactly; an $O(mp^{2})$ Woodbury
form of \eqref{eq:collapsed} (equality with the dense form unit-pinned)
makes the segment-length likelihoods tractable. Inference over the
intonation centre $c$ is by grid --- coarse $1$-cent pass over
$\pm 50$ cents around the \emph{score-nominal} pitch, then a
fine refinement ($0.03$ cents; $0.06$ for the deviation variant) ---
i.e.\ exact 1-D inference up to discretization, with no Laplace
machinery claimed. Four position models
are compared: constant $c$; $c$ plus AR(1) observation noise (residual
whitening, correlation fit per note); $c$ plus the linear drift term of
the \S\ref{sec:phase2} drift study (slope profiled on a grid); and $c$
plus a marginalized pitch-curve deviation prior (eight zero-mean Hann
bumps along the curve Jacobian, prior standard deviation $5$ cents per
coefficient, treated exactly like the amplitudes).
Each posterior is scored against the quasi-truth centre (the Phase-2
NLLS estimator applied to URMP's ground-truth curve): absolute error and
coverage at nominal $90\%$. A fifth condition calibrates the machinery
itself: synthetic notes built from real notes' fitted structure, where
the model is true by construction.

\begin{table}[t]\centering
\caption{Phase-3 development study: waveform-likelihood inference of the
intonation centre on $376$ real notes (seven tracks, three families), no
tracker in the loop. Score: distance to the quasi-truth centre; coverage
at nominal $90\%$. The estimator row is the full pyin-plus-NLLS chain.
Development, exploratory; no claims registered.}
...
\begin{figure}[t]
  ...
  \caption{The Phase-3 study at a glance (development, exploratory).
  \textbf{A}: median distance to the quasi-truth intonation centre falls
  with every position-model refinement, approaching the dedicated
  estimator chain (dashed) --- with no tracker in the loop, and beating
  the estimator on the wind family ($2.67$ vs $3.27$ cents). \textbf{B}:
  coverage against quasi-truth is invariantly near zero for every
  variant, while the same machinery covers at exactly nominal on
  model-true synthetics (green) --- locating the failure in the estimand
  gap, not the inference.}
  \label{fig:phase3-study}
\end{figure}

\paragraph{Findings.} Three facts, visible in
Table~\ref{tab:phase3-study} and Figure~\ref{fig:phase3-study}. First,
\textbf{the waveform carries the information}: median $2.29$ cents from
quasi-truth in the best variant, approaching the estimator chain's
$2.01$ overall and beating it on winds ($2.67$ vs $3.27$), where the
tracker's failure modes live. The two-note pilot that seeded the study
showed the same at $0.7$--$1.6$ cents on long clean notes. Second,
\textbf{every model refinement improves accuracy} --- the drift term of
the \S\ref{sec:phase2} study transfers to the waveform domain (paired
median $-0.18$ cents), and the deviation prior improves it further ---
\textbf{yet coverage against quasi-truth is invariant at $0.01$--$0.03$}:
a better mean model shrinks the posterior width as fast as the error,
and the AR(1) floor fails diagnostically because the structured misfit
is concentrated \emph{in band}, at the harmonics, where a stationary
colored floor cannot reach. Third, \textbf{the machinery itself is
calibrated}: on model-true synthetics the same posterior covers at
exactly $0.90$ (median $|z| = 0.53$).

\paragraph{Diagnosis and design consequence.} Together these locate the
overconfidence precisely: it is an \emph{estimand} gap --- the constant
the harmonic model estimates and the constant the NLLS estimator defines
on the ground-truth curve are different functionals of the same
performance, about two cents apart --- and no within-model refinement can
close a between-question gap, which is why all three failed to. The
design conclusion follows. The waveform posterior should enter the
Phase-2 bundle the way every estimator does: as a measurement whose
noise row is its posterior variance \emph{plus} an empirically
calibrated per-note discrepancy floor at the measured $\sim 2$-cent
scale --- restoring calibration by construction under the as-given
discipline the thesis already runs on. A first integration study
(development, exploratory; same seven tracks) measures exactly this
design: with the waveform posterior as a seventh bundle channel ---
coupling learned per piece by the evidence, discrepancy floor calibrated
from visible notes only (median $3.1$ cents: the estimand-gap scale,
found automatically) --- held-out intonation recovery improves on
\emph{every} track-seed pair ($-1.21$ cents$^{*}$, NLL $-0.24^{*}$,
coverage held), and removing the floor degrades calibration against
quasi-truth significantly ($+0.22^{*}$) while helping nothing --- the
floor doing precisely its designed job. Waveform-native calibration ---
a bridge model for the estimand gap --- is the frontier beyond that.
This measured constraint, not the synthesis sketch, is what the phase's
...
\S\ref{sec:gpfirst} is a Phase-1 evaluation device, not a feature of the method}.
...
edged by specialised variants, e.g.\ corpus-frozen $\tau$ 0.150, chord learned graph $v$ 0.071).}
...
chord learned graph       & 0.152 & 0.94 & 0.601 & 0.90 & 0.071 & 0.90 \\
...
\bibitem{wilson2013} A.~G.~Wilson, R.~P.~Adams.
Gaussian process kernels for pattern discovery and extrapolation.
\emph{ICML}, 2013.
\bibitem{remes2017} S.~Remes, M.~Heinonen, S.~Kaski.
Non-stationary spectral kernels. \emph{NeurIPS}, 2017.
\bibitem{alvarado2016} P.~A.~Alvarado, D.~Stowell.
Gaussian processes for music audio modelling and content analysis.
arXiv:1606.01039, 2016.
