...
verbatim. Phase~3 (audio via a differentiable synthesizer) is scoped,
with a first development study of its waveform likelihood measured: the
audio alone localizes intonation to within about two cents of the
ground-truth-derived centre, and the study fixes the phase's central
design constraint (calibration must bridge the estimand gap).
\end{abstract}
...
(Appendix~\ref{app:phase3}), which are overtones of a single note's fundamental. The two
...
post-confirmation candidates under the $\delta^{\mathrm{vib}}$ template
...
$\delta^{\mathrm{vib}}$ template (adopt as output, claim nothing) if a
...
Features $+$ music-model $+$ harmonic graph (adopted headline) & \rmseHeadline{} & \nllHeadline{} & \covHead \\
...
keeps the plain graph at its operating point; the harmonic edges stand as
...
music-model embeddings enter the kernel as features, the harmonic edge
...
$\delta^{\mathrm{vib}}$ template (included as output, claiming
nothing).
...
chunks of \eqref{eq:loudness}: $p = 2K \times 4 = 64$ columns at $K=8$
