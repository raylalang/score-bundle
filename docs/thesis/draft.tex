...
which the score graph, the hand-built score features, and the music model's
(the small from-scratch symbolic language model of Phase 0) per-note
...
Phase~1 (the score-graph prior and its evaluation) are complete, Phase~2 is
confirmed on real recordings by its own preregistered one-shot (the
secondary timing claim failed and is reported verbatim), and Phase~3 is
scoped, with a first development study of its waveform likelihood
measured.
...
$f_0$-derived channels ($c_i$ cents, vibrato, $\ell_i$) $+$ $\tau$, $d^{\mathrm{vib}}$ & \textbf{Confirmed} (URMP; registered one-shot on the frozen 13-piece pool): intonation recovery, vibrato calibration on both channels, coverage on all six channels; the secondary timing-calibration claim failed, reported verbatim \\
3 --- extension (waveform) & Differentiable synthesizer as audio likelihood &
Harmonic amplitudes $a$, audio $x$ & Scoped; conjugate helpers tested; waveform-likelihood study on 376 real notes (development, exploratory): accuracy competitive with the estimator chain, calibration constraint measured \\
\bottomrule
\end{tabular}
\end{table}
