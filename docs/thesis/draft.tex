...
logs. Figure~\ref{fig:phase2-verdict} shows the procedure and the verdict
at a glance.

\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figures/phase2_verdict.png}
  \caption{The Phase-2 confirmation, at a glance. \textbf{A}: the
  procedure --- the split was frozen data-blind before any Phase-2
  measurement existed; every decision (estimator, noise default, channel
  adoptions, claims) was made on the development side; the frozen pool was
  touched exactly once, on 2026-08-27, under the registered protocol.
  \textbf{B}: each registered claim is a pre-stated paired contrast whose
  $95\%$ interval must exclude zero (as-given vs no-graph; hollow diamond
  = the development basis; rows scaled per claim): C1 and both halves of
  C2 pass, C4 fails --- the interval includes zero --- and is reported.
  \textbf{C}: claim C3, coverage at nominal $90\%$ inside the registered
  band on all six channels.}
  \label{fig:phase2-verdict}
\end{figure}
