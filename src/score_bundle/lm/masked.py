...
The Phase-1 read-out (:func:`masked_note_embeddings_long`) mirrors the training
condition: encode the piece with held-out notes' velocity tokens replaced by ``[MASK]``
and read each note's hidden state at its velocity position.  Import-guarded on torch
like the rest of the LM package.
"""
...
def masked_note_embeddings_long(
    ...
    token is absent from the input), so there is nothing to leak — the Stage-1.5
    strict protocol is the *only* mode this model has.
...
def direct_velocity_mean(
    ...
    estimated from the *observed* notes only (all a real system would have).
    Observed notes keep their true (centered) value; hidden notes get the softmax
    ...
    v = velocities / 127.0 - offset
    v_pred = exp_vel / 127.0 - offset
    return np.where(observed, v, v_pred)
