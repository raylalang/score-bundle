...
def main() -> None:
    ...
    fig = plt.figure(figsize=(10.4, 4.2), dpi=200)
    gs = fig.add_gridspec(4, 1, height_ratios=[2.6, 0.7, 0.7, 0.7], hspace=0.45)
    ...
    ax.set_title("written (outline) vs played (filled)", fontsize=10,
                 color=INK, loc="left")
    ...
    for c in range(3):
        axc = fig.add_subplot(gs[c + 1], sharex=ax)
